...
def main() -> None:
    source_lock_path = PROJECT_ROOT / "configs/v102-presto-context-source-lock.json"
    result_path = PROJECT_ROOT / "outputs/v102-presto-context-source/source-inventory/result.json"
    inventory_path = PROJECT_ROOT / "outputs/v102-presto-context-source/source-inventory/presto-context-dependency-inventory.json"
    doc_path = PROJECT_ROOT / "docs/v102-presto-context-source-results.md"
    verifier_path = PROJECT_ROOT / "python/verify_and_freeze_v102_presto_source_outcome.py"
    audit_path = PROJECT_ROOT / "outputs/v102-presto-context-source/source-outcome-audit.json"
    outcome_path = PROJECT_ROOT / "configs/v102-presto-context-source-outcome-lock.json"
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V102 PRESTO source outcome is already frozen")
    if not doc_path.is_file():
        raise RuntimeError("write the V102 PRESTO source result before freezing")
    ...
    reconstructed = build_presto_context_inventory(source_records, config)
    reconstructed_gates = evaluate_presto_source_gates(reconstructed, config)
    ...
    dependency_kinds = set(config["dependencyRule"]["eligibleContextSources"])
    dependency_previous = {"previous_turn_user_query", "previous_turn_response_text"}
    dependency_seeded = dependency_kinds - dependency_previous
    dependency_keys = (
    ...
    checks = {
        ...
        "archive_identity_is_exact": bool(
            ...
            == result["source_integrity"]["archive_sha256"]
        ...
        "candidate_counts_and_dependency_sources_reconstruct": bool(
            ...
            and sum(bool(set(row["dependency_source_kinds"]) & dependency_previous) for row in rows)
            == inventory["previous_turn_dependent_candidate_count"]
            and sum(bool(set(row["dependency_source_kinds"]) & dependency_seeded) for row in rows)
            == inventory["seeded_state_dependent_candidate_count"]
        ),
        "pairs_and_identifiers_are_unique": bool(
        ...
        "result_gates_and_decision_are_consistent": bool(
            ...
            and result["decision"] == (
                ...
                if result["passed"] else "stop_V102_before_population_language_or_model_access"
            )
        ),
        "text_free_human_only_and_zero_access_boundary_holds": bool(
            ...
            and not inventory["provenance"]["contains_target_arguments_context_tokens_or_seeded_values"]
            and all(
                result["access"][key] == 0
                for key in (
    ...
    audit = {
        "schema_version": "102-presto-context-source-outcome-audit",
        "experiment": "v102_presto_context_source_outcome_audit",
        "passed": integrity_passed,
        "scientific_source_feasibility_passed": result["passed"],
        "decision": (
            "freeze_positive_V102_PRESTO_context_source_feasibility"
            if result["passed"] else "freeze_negative_V102_PRESTO_context_source_feasibility"
        ) if integrity_passed else "reject_V102_PRESTO_source_outcome",
        "checks": checks,
        "independent_summary": {
    ...
    outcome: dict[str, Any] = {
        "schema_version": "102-presto-context-source-outcome-lock",
        "experiment": "v102_presto_context_source_outcome_lock",
        "outcome": {
            "passed": True,
            "scientific_source_feasibility_passed": result["passed"],
        ...
        "authorization": {
            "modify_or_rerun_V102_source_stage": False,
    ...
    outcome["source_archive_payload_sha256"] = hashlib.sha256(source_bytes).hexdigest()
