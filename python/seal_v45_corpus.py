#!/usr/bin/env python3
"""Seal V45 and authorize one paired development run."""
...
def main():
    ...
    parser.add_argument("--audit", default="outputs/v45-delayed-language-grounding/corpus-audit.json")
    parser.add_argument("--manifest", default="data/v45-delayed-language-grounding/manifest.json")
    parser.add_argument("--output", default="configs/v45-corpus-seal.json")
    ...
    if output.exists():
        raise RuntimeError("V45 corpus already sealed")
    audit = json.loads(audit_path.read_text())
    if not audit["passed"] or audit["implementation_lock_sha256"] != file_sha256(lock_path):
        raise RuntimeError("V45 corpus audit did not pass")
    seal = {
        "schema_version": 45,
        "experiment": "v45_corpus_seal",
        ...
        "authorization": {"paired_development_runs": 1, "model_forward_passes": 0, "adapter_training_runs": 0, "final_evaluations": 0},
    }
