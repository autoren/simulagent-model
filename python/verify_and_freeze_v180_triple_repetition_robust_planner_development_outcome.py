...
from run_v180_triple_repetition_robust_planner_development import (
    DEPENDENCY_KEYS,
    reconstruct,
)
...
def main() -> None:
    ...
    result_path = (
        PROJECT_ROOT
        / "outputs/v180-triple-repetition-robust-planner-development/evaluation/result.json"
    )
    doc_path = (
        PROJECT_ROOT
        / "docs/v180-triple-repetition-robust-planner-development-results.md"
    )
    verifier_path = (
        PROJECT_ROOT
        / "python/verify_and_freeze_v180_triple_repetition_robust_planner_development_outcome.py"
    )
    audit_path = (
        PROJECT_ROOT
        / "outputs/v180-triple-repetition-robust-planner-development/outcome-audit.json"
    )
    outcome_path = (
        PROJECT_ROOT
    ...
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V180 outcome is already frozen")
    if not doc_path.is_file():
        raise RuntimeError("write V180 results before freezing")
    ...
    checks = {
        ...
        "scores_summary_and_target_digest_reconstruct_exactly": outputs_exact,
    ...
    audit = {
        "schema_version": "180-triple-repetition-robust-planner-development-outcome-audit",
        ...
        "decision": (
            "freeze_verified_V180_development_outcome"
            if passed
            else "reject_V180_outcome"
        ),
        "checks": checks,
    ...
    dependencies = {
        ...
        "parent_V179_outcome": PROJECT_ROOT / lock["parent_V179_outcome"],
        "source_V177_outcome": PROJECT_ROOT / lock["source_V177_outcome"],
    ...
    outcome: dict[str, Any] = {
        "schema_version": "180-triple-repetition-robust-planner-development-outcome-lock",
        "experiment": config["experiment"],
        "outcome": {
            "passed": True,
            "scientific_safety_gates_passed": safety_passed,
        ...
        "authorization": {
            "modify_rerun_select_subsample_or_tune_V180": False,
            "design_fresh_exact_context_disjoint_robust_confirmation_population": bool(
                safety_passed and beneficial
            ),
            "run_confirmation_without_separate_population_and_lock": False,
            "change_repetition_decoder_cost_horizon_or_unanimity_gate": False,
