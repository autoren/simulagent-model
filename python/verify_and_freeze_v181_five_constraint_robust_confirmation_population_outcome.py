...
from run_v181_five_constraint_robust_confirmation_population import (
    DEPENDENCY_KEYS,
    reconstruct,
)
...
def main() -> None:
    ...
    result_path = (
        PROJECT_ROOT
        / "outputs/v181-five-constraint-robust-confirmation-population/population/result.json"
    )
    doc_path = (
        PROJECT_ROOT
        / "docs/v181-five-constraint-robust-confirmation-population-results.md"
    )
    verifier_path = (
        PROJECT_ROOT
    ...
    audit_path = (
        PROJECT_ROOT
        / "outputs/v181-five-constraint-robust-confirmation-population/outcome-audit.json"
    )
    outcome_path = (
        PROJECT_ROOT
    ...
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V181 outcome is already frozen")
    if not doc_path.is_file():
        raise RuntimeError("write V181 population results before freezing")
    ...
    checks = {
        ...
        "complete_context_disjoint_population_is_frozen_before_scoring": bool(
            population["summary"]["source_state_count"] == 1792
            and population["summary"]["confirmation_eligible_state_count"] == 66
            and population["summary"]["target_case_count"] == 528
            and population["summary"][
        ...
        "candidate_reuse_and_no_effect_boundary_hold": bool(
            population["summary"]["candidate_identity_overlap_with_V172_count"]
            == population["summary"]["unique_target_candidate_count"]
            and population["summary"]["candidate_identity_overlap_with_V176_count"]
            == population["summary"]["unique_target_candidate_count"]
            and all(
                result["access"][key] == 0
                for key in (
    ...
    outcome_audit = {
        "schema_version": "181-five-constraint-robust-confirmation-population-outcome-audit",
        "experiment": config["experiment"],
        "passed": passed,
        "scientific_population_gates_passed": audit["passed"],
        "decision": (
            "freeze_verified_V181_population" if passed else "reject_V181_outcome"
        ),
        "checks": checks,
    ...
    dependencies = {
        ...
        "parent_V180_outcome": PROJECT_ROOT / lock["parent_V180_outcome"],
    }
    for key, item in result["output_integrity"].items():
        dependencies[key] = PROJECT_ROOT / item["path"]
    outcome: dict[str, Any] = {
        "schema_version": "181-five-constraint-robust-confirmation-population-outcome-lock",
        "experiment": config["experiment"],
        "outcome": {
            "passed": True,
            "scientific_population_gates_passed": audit["passed"],
        ...
        "authorization": {
            "modify_or_rebuild_V181": False,
            "preregister_one_unchanged_V180_robust_confirmation": bool(
                audit["passed"]
            ),
            "score_or_run_confirmation_without_separate_lock": False,
            "select_subsample_or_tune_using_V181_outcomes": False,
