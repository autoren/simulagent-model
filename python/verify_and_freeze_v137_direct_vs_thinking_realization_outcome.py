...
from v137_direct_vs_thinking_realization import evaluate_experiment


def main() -> None:
    lock_path = PROJECT_ROOT / "configs/v137-direct-vs-thinking-realization-lock.json"
    result_path = PROJECT_ROOT / "outputs/v137-direct-vs-thinking-realization/model-realization/result.json"
    access_path = PROJECT_ROOT / "outputs/v137-direct-vs-thinking-realization/model-realization/access.json"
    doc_path = PROJECT_ROOT / "docs/v137-direct-vs-thinking-realization-results.md"
    audit_path = PROJECT_ROOT / "outputs/v137-direct-vs-thinking-realization/outcome-audit.json"
    outcome_path = PROJECT_ROOT / "configs/v137-direct-vs-thinking-realization-outcome-lock.json"
    verifier_path = PROJECT_ROOT / "python/verify_and_freeze_v137_direct_vs_thinking_realization_outcome.py"
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V137 outcome already frozen")
    if not doc_path.is_file():
        raise RuntimeError("write V137 results first")
    ...
    audit = {
        "schema_version": "137-direct-vs-thinking-realization-outcome-audit",
    ...
    outcome: dict[str, Any] = {
        "schema_version": "137-direct-vs-thinking-realization-outcome-lock",
        "experiment": "v137_direct_vs_thinking_realization_outcome_lock",
        "outcome": {
            "passed": True,
            "audit_pass": True,
        ...
        "authorization": {
            "modify_retry_rerun_reprompt_or_mine_V137": False,
            "retain_as_synthetic_development_evidence_only": True,
            "preregister_separate_external_transfer_only_if_a_condition_qualified": expected_summary["at_least_one_condition_qualified"],
