...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: run_v220_fresh_mondo_artifact_population.py LOCK")
    ...
    if artifacts["summary"].exists() or artifacts["result"].exists():
        raise RuntimeError("V220 result already exists")
    ...
    if population_manifest["protected_public_sha256"] != file_sha256(artifacts["protectedPublic"]) or population_manifest["protected_truth_sha256"] != file_sha256(artifacts["protectedTruth"]):
        raise RuntimeError("sealed protected file hash mismatch")
    ...
    access = {
        "bounded_retrieval_run_count": 1, "population_build_run_count": 1,
        ...
        "training_run_count": 0, "ontology_registration_count": 0, "trusted_state_mutation_count": 0,
        "service_action_count": 0, "external_side_effect_count_beyond_read_only_retrieval": 0,
        "actual_execution_count": 0,
    }
    audit = audit_population(metrics, access, config)
    summary = {
        "schema_version": "220-fresh-mondo-artifact-population-summary",
    ...
    result = {
        "schema_version": "220-fresh-mondo-artifact-population-result",
        ...
        "authorization": {
            "design_V221_development_only_deterministic_controls": audit["passed"],
