...
def main() -> None:
    ...
    audit_path = PROJECT_ROOT / "outputs/v207r1-agentabstain-shadow-feasibility/outcome-audit.json"
    outcome_path = PROJECT_ROOT / "configs/v207r1-agentabstain-shadow-feasibility-outcome-lock.json"
    results_path = PROJECT_ROOT / "docs/v207r1-agentabstain-shadow-feasibility-results.md"
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V207r1 outcome already frozen")
    ...
    rebuilt = evaluate_feasibility(
        lock["scientific_config_payload"],
        lock["config_payload"],
        dataset_commit=stored_summary["dataset"]["commit"],
    )
    rebuilt_audit = audit_feasibility(rebuilt, lock["scientific_config_payload"], lock["config_payload"])
    summary_exact = stored_summary == rebuilt
    result_path = output_root / "result.json"
    result = json.loads(result_path.read_text())
    scientific_pass = rebuilt["scientific_feasibility_passed"]
    transport_pass = rebuilt["transport_integrity_passed"]
    expected_decision = lock["config_payload"]["decisionRule"][
        "ifTransportIntegrityAndOriginalV207GatesPass"
        if scientific_pass and transport_pass
    ...
    result_exact = bool(
        ...
        and result["transport_integrity_passed"] == transport_pass
    ...
    checks = {
        ...
        "pinned_paginated_metadata_and_schema_reconstruct_exactly": summary_exact,
        ...
        "task_language_model_tool_API_training_authority_side_effect_and_execution_remain_zero": all(
            access[key] == 0
            for key in (
    ...
    outcome_audit = {
        "schema_version": "207r1-agentabstain-shadow-metadata-schema-transport-repair-outcome-audit",
        "experiment": lock["experiment"],
        "passed": passed,
        "scientific_feasibility_passed": scientific_pass,
        "transport_integrity_passed": transport_pass,
        "decision": "freeze_verified_V207r1_feasibility" if passed else "freeze_failed_V207r1_verification",
        "checks": checks,
        "summary": rebuilt,
    }
    ...
    outcome: dict[str, Any] = {
        "schema_version": "207r1-agentabstain-shadow-metadata-schema-transport-repair-outcome-lock",
        "experiment": lock["experiment"],
        "outcome": {
            "passed": True,
            "scientific_feasibility_passed": scientific_pass,
        ...
        "authorization": {
            "preregister_separate_deterministic_text_extraction_only": bool(scientific_pass and transport_pass),
