...
def main() -> None:
    ...
    source_v172_path = (
        PROJECT_ROOT
        / "configs/v172-trusted-shadow-integration-population-outcome-lock.json"
    )
    source_v176_path = (
        PROJECT_ROOT
        / "configs/v176-four-constraint-confirmation-population-outcome-lock.json"
    )
    ...
    plan_path = (
        PROJECT_ROOT
        / "docs/v181-five-constraint-robust-confirmation-population-plan.md"
    )
    protocol_path = (
        PROJECT_ROOT
        / "python/v181_five_constraint_robust_confirmation_population.py"
    )
    tests_path = (
        PROJECT_ROOT
        / "python/test_v181_five_constraint_robust_confirmation_population.py"
    )
    runner_path = (
        PROJECT_ROOT
        / "python/run_v181_five_constraint_robust_confirmation_population.py"
    )
    verifier_path = (
        PROJECT_ROOT
        / "python/verify_and_freeze_v181_five_constraint_robust_confirmation_population_outcome.py"
    )
    auditor_path = (
        PROJECT_ROOT
        / "python/audit_and_freeze_v181_five_constraint_robust_confirmation_population.py"
    )
    audit_path = (
        PROJECT_ROOT
        / "outputs/v181-five-constraint-robust-confirmation-population/design-audit.json"
    )
    lock_path = (
        PROJECT_ROOT
        / "configs/v181-five-constraint-robust-confirmation-population-lock.json"
    )
    output_root = (
        PROJECT_ROOT
    ...
    if audit_path.exists() or lock_path.exists() or output_root.exists() or outcome_path.exists():
        raise RuntimeError("V181 is already preregistered, built, or frozen")
    ...
    V176_states_path = PROJECT_ROOT / v176["constraint_states"]
    V176_targets_path = PROJECT_ROOT / v176["target_cases"]
    ...
    checks = {
        "V180_is_frozen_strong_positive_and_authorizes_only_separate_population_design": bool(
            valid_lock(parent)
            ...
            and parent["authorization"][
                "design_fresh_exact_context_disjoint_robust_confirmation_population"
            ]
            and not parent["authorization"][
                "run_confirmation_without_separate_population_and_lock"
            ]
            and not parent["authorization"]["modify_rerun_select_subsample_or_tune_V180"]
        ),
        "V172_V176_source_artifacts_and_V167_prior_are_exact": bool(
            valid_lock(v172)
            and valid_lock(v176)
            ...
            and all(
                path.is_file() and file_sha256(path) == outcome[f"{key}_sha256"]
                for outcome, key, path in (
                    (v172, "constraint_states", V172_states_path),
                    (v172, "target_cases", V172_targets_path),
                    (v176, "constraint_states", V176_states_path),
                    (v176, "target_cases", V176_targets_path),
                )
            )
        ),
        "complete_five_constraint_generator_is_frozen_without_score_selection": bool(
            generator["candidateTruthTableCount"] == 256
            and generator["valuationCount"] == 8
            and generator["constraintCountPerState"] == 5
            and generator["outcomesPerConstraint"] == [0, 1]
            and generator["completeValuationQuintupleAndOutcomeEnumeration"]
            and generator["requiredSourceStateCount"] == 1792
            and generator["retainEverySourceState"]
            and not generator[
                "selectionUsesPlannerScoresPoliciesSandboxOutcomesOrTargetClass"
            ]
        ),
        ...
        "freshness_is_context_disjoint_with_candidate_reuse_disclosed": bool(
            freshness["compareAgainstEveryV172TargetContextSignature"]
            and freshness["compareAgainstEveryV176TargetContextSignature"]
            and freshness["requiredExactSignatureOverlapWithV172"] == 0
            and freshness["requiredExactSignatureOverlapWithV176"] == 0
        ...
        "population_gates_match_prelock_structural_census_and_zero_scores": bool(
            gates["requiredSourceStateCount"] == 1792
            and gates["requiredSelectedStateCount"] == 1792
            and gates["requiredCandidatesPerState"] == 8
            and gates["requiredEligibleStateCount"] == 66
            and gates["requiredIneligibleStateCount"] == 1726
            and gates["requiredTargetCaseCount"] == 528
            and gates["requiredTargetClassCounts"]
            == {"alias": 66, "composition": 72, "provisional_primitive": 390}
            and gates["requiredClassCoverageCounts"]
            == {"1": 1388, "2": 338, "3": 66}
            and gates["requiredExactTargetContextSignatureOverlapWithV172"] == 0
            and gates["requiredExactTargetContextSignatureOverlapWithV176"] == 0
        ...
        "prelock_and_authority_boundaries_are_closed": bool(
            ...
            and authority[
                "populationContainsOnlyShadowVersionSpacesAndTargetIdentities"
            ]
            ...
            and not config["decisionRule"][
                "passAuthorizesModelRegistrationAuthorityActionOrExecution"
            ]
        ),
    ...
    audit = {
        "schema_version": "181-five-constraint-robust-confirmation-population-design-audit",
        "experiment": config["experiment"],
        "passed": passed,
        "decision": (
            "freeze_and_authorize_one_formal_V181_population_build"
            if passed
            else "reject_V181_design"
        ),
        "checks": checks,
    ...
    dependencies = {
        "config": config_path,
        "parent_V180_outcome": parent_path,
        ...
        "V176_constraint_states": V176_states_path,
        "V176_target_cases": V176_targets_path,
    ...
    lock: dict[str, Any] = {
        "schema_version": "181-five-constraint-robust-confirmation-population-lock",
        "experiment": config["experiment"],
        ...
        "authorization": {
            ...
            "run_model_register_mutate_state_call_sensor_service_or_execute": False,
        },
    }
