...
def main():
    ...
    parser.add_argument("--output", default="configs/v52r2-outcome-lock.json")
    ...
    if output.exists():
        raise RuntimeError("V52r2 outcome already frozen")
    result = json.loads(result_path.read_text())
    audit = json.loads(audit_path.read_text())
    if not audit["passed"] or audit["result_sha256"] != file_sha256(result_path):
        raise RuntimeError("V52r2 post-result audit is not bound to result")
    lock = {
        "schema_version": 52,
        "revision": "r2",
        "experiment": "v52r2_outcome_lock",
        ...
        "repair_boundary_checks": audit["repair_boundary_checks"],
