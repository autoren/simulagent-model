...
from run_v179_triple_repetition_robust_feasibility import (
    DEPENDENCY_KEYS,
    reconstruct,
    terminal_completion,
)
from v179_triple_repetition_robust_feasibility import evaluate_gates
...
def main() -> None:
    ...
    result_path = PROJECT_ROOT / "outputs/v179-triple-repetition-robust-feasibility/census/result.json"
    doc_path = PROJECT_ROOT / "docs/v179-triple-repetition-robust-feasibility-results.md"
    verifier_path = PROJECT_ROOT / "python/verify_and_freeze_v179_triple_repetition_robust_feasibility_outcome.py"
    audit_path = PROJECT_ROOT / "outputs/v179-triple-repetition-robust-feasibility/outcome-audit.json"
    outcome_path = PROJECT_ROOT / "configs/v179-triple-repetition-robust-feasibility-outcome-lock.json"
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V179 outcome is already frozen")
    if not doc_path.is_file():
        raise RuntimeError("write V179 results before freezing")
    ...
    if passed_gates and positive:
        expected_decision = config["decisionRule"]["ifEveryGatePassesAndTargetBlindTrustedCompletionIsPositive"]
    elif passed_gates:
        expected_decision = config["decisionRule"]["ifEveryGatePassesButTargetBlindTrustedCompletionIsZero"]
    else:
    ...
    expected_outputs = {
        ...
        "state_adaptive_opportunity_results": {"state_results": evaluation["state_results"], "policy_cost_scored": False},
    ...
    checks = {
        ...
        "gates_feasibility_and_decision_reconstruct_exactly": bool(
            ...
            and result["triple_repetition_target_blind_robust_feasibility_positive"] == positive
            and result["decision"] == expected_decision
        ),
        "complete_repetition_code_census_is_cost_and_sandbox_free": bool(
            evaluation["summary"]["state_count"] == 135
            and evaluation["summary"]["target_count"] == 2160
            and evaluation["summary"]["full_measurement_adversarial_scenario_count"] == 28080
            and result["access"]["planner_risk_or_cost_score_count"] == 0
            and result["access"]["sandbox_transaction_count"] == 0
        ),
        "decode_equivalence_unanimity_and_no_effect_boundary_hold": bool(
            evaluation["summary"]["majority_decode_exactness_rate"] == 1.0
            and evaluation["summary"]["robust_clean_version_space_equivalence_rate"] == 1.0
            ...
            and all(
                ...
                for key in (
                    ...
                    "ontology_registration_count", "trusted_real_state_mutation_count",
    ...
    outcome_audit = {
        "schema_version": "179-triple-repetition-robust-feasibility-outcome-audit",
        "experiment": config["experiment"],
        "passed": passed,
        "scientific_feasibility_gates_passed": passed_gates,
        "triple_repetition_target_blind_robust_feasibility_positive": positive,
        "decision": "freeze_verified_V179_structural_boundary" if passed else "reject_V179_outcome",
        "checks": checks,
        "independent_summary": evaluation["summary"],
        "additional_access": {"planner_risk_or_cost_score_count": 0, "sandbox_transaction_count": 0, "model_load_count": 0, "actual_execution_count": 0},
    }
    ...
    dependencies = {
        ...
        "parent_V178_outcome": PROJECT_ROOT / lock["parent_V178_outcome"],
    }
    for key, item in result["output_integrity"].items():
        dependencies[key] = PROJECT_ROOT / item["path"]
    outcome: dict[str, Any] = {
        "schema_version": "179-triple-repetition-robust-feasibility-outcome-lock",
        "experiment": config["experiment"],
        "outcome": {
            "passed": True,
            "scientific_feasibility_gates_passed": passed_gates,
        ...
        "authorization": {
            "modify_rerun_select_subsample_or_tune_V179": False,
            "design_separate_triple_repetition_robust_planner_development": bool(passed_gates and positive),
            "run_planner_without_separate_design_lock": False,
