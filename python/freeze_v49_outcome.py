...
def main():
    ...
    parser.add_argument("--audit", default="outputs/v49-passive-partial-observation/post-result-audit.json")
    parser.add_argument("--output", default="configs/v49-outcome-lock.json")
    args = parser.parse_args()
    result_path, audit_path, output = tuple(
        (PROJECT_ROOT / value).resolve() for value in (args.result, args.audit, args.output)
    )
    if output.exists():
        raise RuntimeError("V49 outcome already frozen")
    result = json.loads(result_path.read_text())
    audit = json.loads(audit_path.read_text())
    if not audit["passed"] or audit["result_sha256"] != file_sha256(result_path):
        raise RuntimeError("V49 post-result audit is not bound to result")
    lock = {
        "schema_version": 49,
        "experiment": "v49_outcome_lock",
