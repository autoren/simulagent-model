...
def main() -> None:
    source_lock_path = PROJECT_ROOT / "configs/v96-two-source-open-set-source-lock.json"
    result_path = PROJECT_ROOT / "outputs/v96-two-source-open-set/source-inventory/result.json"
    inventory_path = PROJECT_ROOT / "outputs/v96-two-source-open-set/source-inventory/two-source-open-set-inventory.json"
    doc_path = PROJECT_ROOT / "docs/v96-two-source-open-set-source-results.md"
    verifier_path = PROJECT_ROOT / "python/verify_and_freeze_v96_source_outcome.py"
    audit_path = PROJECT_ROOT / "outputs/v96-two-source-open-set/source-outcome-audit.json"
    outcome_path = PROJECT_ROOT / "configs/v96-two-source-open-set-source-outcome-lock.json"
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V96 source outcome is already frozen")
    if not doc_path.is_file():
        raise RuntimeError("write the V96 source result before freezing")
    lock = json.loads(source_lock_path.read_text())
    result = json.loads(result_path.read_text())
    inventory = json.loads(inventory_path.read_text())
    config = lock["config_payload"]
    catalog_path = PROJECT_ROOT / result["source_integrity"]["catalog"]["local_path"]
    unsupported_path = PROJECT_ROOT / result["source_integrity"]["unsupported"]["local_path"]
    catalog_bytes = catalog_path.read_bytes()
    unsupported_bytes = unsupported_path.read_bytes()
    schema_path = PROJECT_ROOT / config["schemaDependency"]["localPath"]
    reconstructed = build_two_source_open_set_inventory(
        json.loads(schema_path.read_text()),
        json.loads(catalog_bytes),
        json.loads(unsupported_bytes),
        config,
    )
    ...
    identities_exact = all(
        len(data) == config[config_key]["byteSize"]
        and git_blob_sha1(data) == config[config_key]["gitBlobSha1"]
        and file_sha256(path) == result["source_integrity"][role]["local_sha256"]
        for role, config_key, path, data in (
            ("catalog", "catalogDialogueShard", catalog_path, catalog_bytes),
            ("unsupported", "unsupportedDialogueShard", unsupported_path, unsupported_bytes),
        )
    )
    checks = {
        ...
        "both_source_identities_are_exact": identities_exact,
        ...
        "source_roles_are_disjoint_and_rows_respect_roles": bool(
            not (set(inventory["catalog_services"]) & set(inventory["unsupported_services"]))
            and all(
                row["source_role"] == ("unsupported" if row["class_label"] == "unsupported" else "catalog")
                for row in records
            )
        ),
        "partition_is_disjoint_and_service_stratified": bool(
            not (set(inventory["hidden_pairs"]) & set(inventory["declared_supported_pairs"]))
        ...
        "result_and_gate_decision_are_consistent": bool(
            ...
            and result["decision"] == (
                "freeze_source_and_preregister_two_source_open_set_population"
                if result["passed"] else "stop_V96_before_population_or_model_access"
            )
        ),
    ...
    audit = {
        "schema_version": "96-two-source-open-set-source-outcome-audit",
        "experiment": "v96_two_source_open_set_source_outcome_audit",
        "passed": integrity_passed,
        "scientific_source_feasibility_passed": result["passed"],
        "decision": (
            "freeze_positive_V96_two_source_feasibility" if result["passed"]
            else "freeze_negative_V96_two_source_feasibility"
        ) if integrity_passed else "reject_V96_source_outcome",
        "checks": checks,
        "independent_summary": {
    ...
    dependencies = {
        ...
        "unsupported_source_file": unsupported_path,
        "verifier": verifier_path,
        "audit": audit_path,
        "results_document": doc_path,
    }
    outcome: dict[str, Any] = {
        "schema_version": "96-two-source-open-set-source-outcome-lock",
        "experiment": "v96_two_source_open_set_source_outcome_lock",
        "outcome": {
            "passed": True,
            "scientific_source_feasibility_passed": result["passed"],
        ...
        "authorization": {
            "modify_or_rerun_V96_source_stage": False,
    ...
    outcome["catalog_source_file_git_blob_sha1"] = git_blob_sha1(catalog_bytes)
    outcome["unsupported_source_file_git_blob_sha1"] = git_blob_sha1(unsupported_bytes)
