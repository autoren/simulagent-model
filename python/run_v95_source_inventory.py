...
from v95_activation_open_set_source import build_activation_open_set_inventory, git_blob_sha1
...
def main() -> None:
    ...
    inventory_root = PROJECT_ROOT / "outputs/v95-activation-open-set/source-inventory"
    if source_root.exists() or inventory_root.exists():
        raise RuntimeError("V95 source inventory may run only once")
    lock = json.loads(lock_path.read_text())
    if payload_hash({key: value for key, value in lock.items() if key != "lock_payload_sha256"}) != lock["lock_payload_sha256"]:
        raise RuntimeError("V95 source lock mismatch")
    dependency_keys = (
    ...
    for key in dependency_keys:
        if file_sha256(PROJECT_ROOT / lock[key]) != lock[f"{key}_sha256"]:
            raise RuntimeError(f"V95 dependency drifted: {key}")

    config = lock["config_payload"]
    shard = config["newDialogueShard"]
    request = Request(shard["rawUrl"], headers={"User-Agent": "simulagent-v95-activation-open-set"})
    ...
    if len(data) != shard["byteSize"] or git_blob_sha1(data) != shard["gitBlobSha1"]:
        raise RuntimeError("V95 pinned shard identity mismatch")
    schema_path = PROJECT_ROOT / config["schemaDependency"]["localPath"]
    if file_sha256(schema_path) != config["schemaDependency"]["localSha256"]:
        raise RuntimeError("V95 schema dependency drifted")
    inventory = build_activation_open_set_inventory(
    ...
    inventory_path = inventory_root / "activation-open-set-inventory.json"
    ...
    summary_keys = (
        "source_record_count", "source_intent_activation_count", "eligible_fresh_services",
        "eligible_fresh_service_count", "catalog_services", "catalog_service_count",
        "unsupported_services", "unsupported_service_count", "supported_pairs",
        "supported_pair_count", "hidden_services", "hidden_pairs", "hidden_pair_count",
        "declared_supported_pairs", "declared_supported_pair_count", "complete_catalog_pair_count",
        "candidate_count", "class_counts", "class_service_counts", "candidate_index_sha256",
    ...
    result = {
        "schema_version": "95-activation-open-set-source-result",
        "experiment": "v95_activation_open_set_source_inventory",
        "passed": passed,
        "decision": "freeze_source_and_preregister_activation_open_set_population" if passed else "stop_V95_before_population_or_model_access",
        "source_integrity": {
            "path": shard["path"],
            "byte_size": len(data),
        ...
        "access": {
            ...
            "automatic_current_turn_tokenization_count": inventory["source_record_count"],
        ...
        "claim_boundary": "automatic text-free activation-source feasibility only; no selected language, model, calibration, posterior, planning, or execution outcome",
    }
