...
def main():
    ...
    parser.add_argument("--audit", default="outputs/v49-passive-partial-observation/corpus-audit.json")
    parser.add_argument("--output", default="configs/v49-corpus-seal.json")
    ...
    if output.exists():
        raise RuntimeError("V49 corpus already sealed")
    lock = json.loads(lock_path.read_text())
    audit = json.loads(audit_path.read_text())
    if not audit["passed"] or audit["implementation_lock_sha256"] != file_sha256(lock_path):
        raise RuntimeError("V49 corpus audit is not bound to implementation")
