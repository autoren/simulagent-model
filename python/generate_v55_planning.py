#!/usr/bin/env python3
"""Construct the sealed V55 short-horizon planning task population."""
...
def history_class_for_record(record):
    return "prior_like_all_wait" if record % 2 == 0 else "mixed_informative"


def target_assignments(config, registry_size=8):
    count = config["population"]["planningTasks"]
    if count % registry_size:
        raise RuntimeError("V55 task count must divide evenly across templates")
    values = [
        index for index in range(registry_size)
        for _ in range(count // registry_size)
    ]
    random.Random(config["population"]["truthAssignmentSeed"]).shuffle(values)
    return values


def goal_values(config):
    count = config["population"]["planningTasks"]
    if count % 2:
        raise RuntimeError("V55 task count must be even for exact Boolean balance")
    values = [False] * (count // 2) + [True] * (count // 2)
    random.Random(config["population"]["goalSeed"]).shuffle(values)
    return values


def goal_for_record(record, entity_rows, config, value=None):
    atoms = sorted(atom_universe(entity_rows))
    atom = min(atoms, key=lambda item: sha256_text(
        f"v55-goal|{config['population']['goalSeed']}|{record}|{item}"
    ))
    if value is None:
        value = goal_values(config)[record]
    return {"atom": atom, "value": bool(value)}


def mask_schedule(entity_rows, steps, token, config):
    ...
    return [
        sorted(sorted(atoms, key=lambda atom: sha256_text(
            f"v55-mask|{seed}|{token}|{tick}|{atom}"
        ))[:count])
        for tick in range(steps)
    ]


def action_schedule(entity_rows, steps, token, history_class, config):
    ...
    for tick in range(steps):
        action_id = ("pulse", "route", "wait")[
            int(sha256_text(f"v55-action|{seed}|{token}|{tick}")[:12], 16) % 3
        ]
        if action_id == "wait":
            actions.append({"id": "wait", "binding": {}})
        else:
            index = int(sha256_text(
                f"v55-binding|{seed}|{token}|{tick}"
            )[:12], 16) % len(bindings)
            actions.append({"id": action_id, "binding": dict(bindings[index])})
    if steps:
...
def make_case(record, ordinal, length, history_class, config, kind, used, prior):
    nonce = 0
    while True:
        token = (
            f"v55|{config['population']['generatorSeed']}|{kind}|{record}|"
...
def simulate_case(target, theta, case, config, token):
...
def record_theta(record, config):
    return scaled_beta_sample(stream_seed(
        config["population"]["thetaPriorSeed"], "planning", record, "theta-prior"
    ))


def build_record(
    record, target_index, goal_value, registry, config, used, prior
):
    ...
    for ordinal in range(config["population"]["supportEpisodesPerTask"]):
        ...
        case = make_case(
            record, ordinal, length, history_class, config, "support", used, prior
        )
        sampled = simulate_case(
            target, theta, case, config, f"planning|{record}|support|{ordinal}"
        )
    ...
    case = make_case(
        record, 0, prefix, history_class, config, "query", used, prior
    )
    sampled = simulate_case(
        target, theta, case, config, f"planning|{record}|query"
    )
    query = public_case(case, sampled)
    query["prefix_length"] = prefix
    goal = goal_for_record(record, query["entities"], config, goal_value)
    public = {"supports": supports, "query": query, "goal": goal}
    assert_planning_payload_is_public(public)
    return {
        "id": f"planning_{record:05d}",
        "schema_version": 55,
        "population": "planning",
        "record": record,
...
def build_population(config):
    ...
    return [
        build_record(record, assignments[record], values[record], registry, config, used, prior)
        for record in range(config["population"]["planningTasks"])
    ]
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="data/v55-short-horizon-bayes-adaptive-planning"
    )
    args = parser.parse_args()
    lock_path = (PROJECT_ROOT / args.implementation_lock).resolve()
    lock = json.loads(lock_path.read_text())
    if not lock["authorization"]["construct_v55_planning_population"]:
        raise RuntimeError("V55 implementation lock does not authorize population construction")
    for relative, digest in lock["implementation_files_sha256"].items():
        if file_sha256(PROJECT_ROOT / relative) != digest:
            raise RuntimeError(f"V55 frozen implementation changed: {relative}")
    ...
    manifest = {
        "schema_version": 55,
        "experiment": "v55_planning_population_manifest",
        "count": len(rows),
        "file": {
