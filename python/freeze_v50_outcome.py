...
def main():
    ...
    parser.add_argument("--audit", default="outputs/v50-history-dependent-belief-filtering/post-result-audit.json")
    parser.add_argument("--output", default="configs/v50-outcome-lock.json")
    args = parser.parse_args()
    result_path, audit_path, output = tuple(
        (PROJECT_ROOT / value).resolve() for value in (args.result, args.audit, args.output)
    )
    if output.exists():
        raise RuntimeError("V50 outcome already frozen")
    result = json.loads(result_path.read_text())
    audit = json.loads(audit_path.read_text())
    if not audit["passed"] or audit["result_sha256"] != file_sha256(result_path):
        raise RuntimeError("V50 post-result audit is not bound to result")
    lock = {
        "schema_version": 50,
        "experiment": "v50_outcome_lock",
