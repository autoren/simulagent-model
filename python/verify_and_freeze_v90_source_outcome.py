#!/usr/bin/env python3
"""Independently verify and freeze the V90 source-extension outcome."""
...
def main() -> None:
    source_lock_path = PROJECT_ROOT / "configs/v90-capacity-generation-source-lock.json"
    result_path = PROJECT_ROOT / "outputs/v90-capacity-generation/source-inventory/result.json"
    inventory_path = PROJECT_ROOT / "outputs/v90-capacity-generation/source-inventory/structural-inventory.json"
    doc_path = PROJECT_ROOT / "docs/v90-capacity-generation-source-results.md"
    verifier_path = PROJECT_ROOT / "python/verify_and_freeze_v90_source_outcome.py"
    audit_path = PROJECT_ROOT / "outputs/v90-capacity-generation/source-outcome-audit.json"
    outcome_lock_path = PROJECT_ROOT / "configs/v90-capacity-generation-source-outcome-lock.json"
    if audit_path.exists() or outcome_lock_path.exists():
        raise RuntimeError("V90 source outcome is already frozen")
    if not doc_path.is_file():
        raise RuntimeError("write the source-only results document before freezing the outcome")

    source_lock = json.loads(source_lock_path.read_text())
    source_lock_payload = {key: value for key, value in source_lock.items() if key != "lock_payload_sha256"}
    ...
    forbidden = {"utterance", "text", "slot_values", "values", "question", "prompt", "dialogue_history"}
    ...
    checks = {
        "source_lock_and_all_frozen_dependencies_are_exact": bool(
            ...
            and all(file_sha256(PROJECT_ROOT / source_lock[key]) == source_lock[f"{key}_sha256"] for key in (
                "config", "parent_outcome_lock", "plan", "inventory_module", "runner", "verifier", "auditor"
            ))
        ),
        ...
        "structural_aggregates_reconstruct": bool(
            ...
            and counts["active"] == inventory["counts"]["eligible_active_record_count"]
            and counts["none"] == inventory["counts"]["eligible_none_record_count"]
            and dict(sorted(service_counts.items())) == inventory["eligible_service_counts"]
        ...
        "zero_manual_model_API_training_service_or_side_effect_access": all(
            result["access"][key] == 0 for key in (
                "manual_utterance_inspection_count", "model_weight_download_count", "model_load_count",
        ...
        "license_and_source_provenance_are_preserved": bool(
            ...
            and inventory["provenance"]["source_shard"] == config["newDialogueShard"]["path"]
    ...
    audit = {
        "schema_version": "90-capacity-generation-source-outcome-audit",
        "experiment": "v90_capacity_generation_source_outcome_audit",
        "passed": passed,
        "decision": "freeze_positive_fresh_source_inventory" if passed else "reject_V90_source_outcome",
        "checks": checks,
    ...
    lock = {
        "schema_version": "90-capacity-generation-source-outcome-lock",
        "experiment": "v90_capacity_generation_source_outcome_lock",
        ...
        "authorization": {
            ...
            "preregister_fresh_hash_selected_multi_model_shadow_population": True,
            "select_population_before_utterance_extraction": True,
            "manually_inspect_source_language": False,
            "download_or_run_model_before_population_prompt_and_gates_lock": False,
