#!/usr/bin/env python3
"""Audit and freeze the one-shot V74 development evaluator."""
...
def main() -> None:
    structural_lock_path = PROJECT_ROOT / "configs/v74-active-sensing-structural-lock.json"
    config_path = PROJECT_ROOT / "configs/v74-active-sensing-development-evaluation.json"
    plan_path = PROJECT_ROOT / "docs/v74-active-sensing-development-evaluation-plan.md"
    ...
    evaluator_path = PROJECT_ROOT / "python/evaluate_v74_pomdppy_tiger_development.py"
    auditor_path = PROJECT_ROOT / "python/audit_and_freeze_v74_development_evaluator.py"
    audit_path = PROJECT_ROOT / "outputs/v74-active-sensing/development-evaluator-audit.json"
    lock_path = PROJECT_ROOT / "configs/v74-active-sensing-development-evaluator-lock.json"
    if lock_path.exists():
        raise RuntimeError("V74 development evaluator is already frozen")
    ...
    authorization_ok = bool(
        ...
        and structural_lock["authorization"][
            "implement_and_lock_development_evaluator"
        ]
        and not structural_lock["authorization"]["run_exact_development_evaluation"]
    )
    if not authorization_ok:
        errors.append("V74 structural lock does not authorize evaluator-only work")

    gates = config["gates"]
    config_ok = bool(
        config["modelCount"] == 1
        and config["horizonActions"] == 3
        and gates["requiredExactRootAction"] == "calibrate_beacon"
        and gates["requiredMAPRootAction"] == "listen_target"
        and gates["minimumNormalizedMAPRegret"] == 0.1
        and gates["minimumNormalizedPosteriorSamplingRegret"] == 0.1
        and gates["minimumNormalizedExactOverOpenLoopAdvantage"] == 0.015
        and gates["minimumNormalizedMyopicRegret"] == 0.015
        and config["decisionRule"]["noRetrospectiveChanges"] is True
        and not config["claimBoundary"]["externalConfirmationEvidence"]
    )
    if not config_ok:
        errors.append("V74 development gates or stop rule drifted")
    ...
    if not tests_ok:
        errors.append(f"V74 source tests failed: {combined[-1600:]}")

    evaluator_source = evaluator_path.read_text()
    source_checks = {
        "uses_locked_exact_planner": "from v71_exact_planning import" in evaluator_source,
        "uses_locked_tiger_exporter": "from v74_pomdppy_tiger_source import" in evaluator_source,
        ...
        "separate_development_output": "outputs/v74-active-sensing/development-evaluation"
        in evaluator_source,
        "protected_firewall": "protected_confirmation_policy_value_count"
        in evaluator_source,
        "prior_protected_firewall": "prior_protected_access_count" in evaluator_source,
        "EIG_firewall": "candidate_EIG_value_count" in evaluator_source,
        "stop_rule_present": "freeze_negative_result_and_stop_V74_before_protected_discovery"
        in evaluator_source,
    ...
    if not all(source_checks.values()):
        errors.append("V74 evaluator expands beyond the registered screen")

    outcome_absent = not (
        PROJECT_ROOT / "outputs/v74-active-sensing/development-evaluation"
    ).exists()
    if not outcome_absent:
        errors.append("V74 development outcome predates evaluator lock")
    ...
    audit = {
        ...
        "experiment": "v74_development_evaluator_audit",
        "passed": not errors and all(checks.values()),
        "decision": (
            "freeze_evaluator_and_authorize_one_development_run"
            if not errors
            else "reject_v74_development_evaluator"
        ),
        "errors": errors,
        "checks": checks,
    ...
    lock = {
        "schema_version": "74-active-sensing-development-evaluation",
        "experiment": "v74_pomdppy_tiger_development_evaluator_lock",
        ...
        "attempt_path": "outputs/v74-active-sensing/development-evaluation/attempt.json",
        "result_path": "outputs/v74-active-sensing/development-evaluation/result.json",
        ...
        "authorization": {
            ...
            "read_prior_protected_models_histories_or_outcomes": False,
            "run_SMC2": False,
            "human_data": False,
            "model_access": False,
