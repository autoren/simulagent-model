...
from run_v177_certification_aware_planner_fresh_confirmation import (
    DEPENDENCY_KEYS,
    reconstruct,
)
...
def main() -> None:
    ...
    result_path = PROJECT_ROOT / "outputs/v177-certification-aware-planner-fresh-confirmation/confirmation/result.json"
    doc_path = PROJECT_ROOT / "docs/v177-certification-aware-planner-fresh-confirmation-results.md"
    verifier_path = PROJECT_ROOT / "python/verify_and_freeze_v177_certification_aware_planner_fresh_confirmation_outcome.py"
    audit_path = PROJECT_ROOT / "outputs/v177-certification-aware-planner-fresh-confirmation/outcome-audit.json"
    outcome_path = PROJECT_ROOT / "configs/v177-certification-aware-planner-fresh-confirmation-outcome-lock.json"
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V177 outcome is already frozen")
    if not doc_path.is_file():
        raise RuntimeError("write V177 results before freezing")
    ...
    if safety_passed and confirmed and strong_confirmation:
        ...
        expected_decision = config["decisionRule"]["ifSafetyAndPrimaryPassButStrongFails"]
    elif safety_passed:
        expected_decision = config["decisionRule"]["ifSafetyPassesButPrimaryFails"]
    else:
    ...
    checks = {
        ...
        "complete_fresh_population_policy_and_DP_reconstruction_hold": bool(
            ...
            and evaluation["summary"]["target_policy_score_count"] == 15120
    ...
    outcome_audit = {
        "schema_version": "177-certification-aware-planner-fresh-confirmation-outcome-audit",
        "experiment": config["experiment"],
        "passed": passed,
        "scientific_safety_gates_passed": safety_passed,
        "confirmed": confirmed,
        "strong_confirmation": strong_confirmation,
        "decision": "freeze_verified_V177_confirmation" if passed else "reject_V177_outcome",
        "checks": checks,
    ...
    dependencies = {
        ...
        "parent_V176_outcome": PROJECT_ROOT / lock["parent_V176_outcome"],
        "source_V175_outcome": PROJECT_ROOT / lock["source_V175_outcome"],
    ...
    outcome: dict[str, Any] = {
        "schema_version": "177-certification-aware-planner-fresh-confirmation-outcome-lock",
        "experiment": config["experiment"],
        "outcome": {
            "passed": True,
            "scientific_safety_gates_passed": safety_passed,
        ...
        "authorization": {
            "modify_rerun_select_subsample_or_tune_V177": False,
            "design_separate_exact_robustness_branch": bool(
                safety_passed and confirmed
            ),
            "run_robustness_without_separate_population_and_lock": False,
