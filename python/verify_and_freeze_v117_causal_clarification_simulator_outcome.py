...
from audit_and_freeze_v117_causal_clarification_simulator import payload_hash
...
def main() -> None:
    ...
    doc_path = PROJECT_ROOT / "docs/v117-causal-clarification-simulator-results.md"
    audit_path = PROJECT_ROOT / "outputs/v117-causal-clarification-simulator/outcome-audit.json"
    outcome_path = PROJECT_ROOT / "configs/v117-causal-clarification-simulator-outcome-lock.json"
    verifier_path = PROJECT_ROOT / "python/verify_and_freeze_v117_causal_clarification_simulator_outcome.py"
    if audit_path.exists() or outcome_path.exists(): raise RuntimeError("V117 outcome already frozen")
    if not doc_path.is_file(): raise RuntimeError("write V117 result document before freezing")
    ...
    passed = all(checks.values()); audit = {"schema_version": "117-causal-clarification-simulator-outcome-audit", "experiment": lock["config_payload"]["experiment"], "passed": passed, "checks": checks, "independent_summary": summary}
    ...
    outcome: dict[str, Any] = {"schema_version": "117-causal-clarification-simulator-outcome-lock", "experiment": "v117_causal_clarification_simulator_outcome_lock", "outcome": {"passed": True, "simulator_pass": summary["outcome_pass"], "decision": summary["decision"], "summary": summary}, "authorization": {"modify_rerun_or_retune_V117": False, "preregister_fresh_unprotected_multiturn_language_protocol": bool(summary["outcome_pass"]), "run_language_or_model_immediately": False, "open_protected_test": False, "begin_schema_induction_or_richer_planning": False, "run_API_or_training": False, "grant_capability_belief_action_or_execution_authority": False}}
