...
def reconstruct(lock: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    V176_states = json.loads(
        (PROJECT_ROOT / lock["V176_constraint_states"]).read_text()
    )
    V176_targets = json.loads(
        (PROJECT_ROOT / lock["V176_target_cases"]).read_text()
    )
...
def main() -> None:
    ...
    if output_root.exists():
        raise RuntimeError("V181 formal population may be built only once")
    ...
    if (
        ...
        raise RuntimeError("V181 lock mismatch")
    for key in DEPENDENCY_KEYS:
        if file_sha256(PROJECT_ROOT / lock[key]) != lock[f"{key}_sha256"]:
            raise RuntimeError(f"V181 dependency drifted: {key}")
    ...
    result = {
        "schema_version": "181-five-constraint-robust-confirmation-population-result",
