...
def main() -> None:
    ...
    for key in dependency_keys:
        if file_sha256(PROJECT_ROOT / lock[key]) != lock[f"{key}_sha256"]:
            raise RuntimeError(f"V104 dependency drifted: {key}")
    ...
    artifacts = build_selected_language_artifacts(population, source_inventory, source_records, config)
    ...
    summary_keys = (
        "total_record_count", "role_record_counts", "role_class_record_counts",
        ...
        "development_protected_test_disjoint", "unselected_language_record_count",
        "role_payload_sha256",
    )
    result = {
        "schema_version": "104-massive-language-extraction-result",
        "experiment": "v104_massive_selected_language_extraction",
        "passed": passed,
        "decision": "freeze_language_and_preregister_benchmark_interface" if passed else "stop_V104_before_prompt_or_model_access",
        ...
        "access": {
            ...
            "selected_language_record_extraction_count": artifacts["total_record_count"],
        ...
        "claim_boundary": "exact automatic selected-language extraction only; no baseline, model, calibration, posterior, planning, or execution outcome",
    }
