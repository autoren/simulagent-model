...
def reconstruct(lock: dict[str, Any]) -> dict[str, Any]:
    ...
    certificates = json.loads((PROJECT_ROOT / lock["target_certificate_results"]).read_text())
    objective = lock["config_payload"]["frozenRoutedObjective"]
...
def main() -> None:
    ...
    if output_root.exists():
        raise RuntimeError("V175 formal development evaluation may run only once")
    lock = json.loads(lock_path.read_text())
    if payload_hash({key: value for key, value in lock.items() if key != "lock_payload_sha256"}) != lock["lock_payload_sha256"]:
        raise RuntimeError("V175 lock mismatch")
    for key in DEPENDENCY_KEYS:
        if file_sha256(PROJECT_ROOT / lock[key]) != lock[f"{key}_sha256"]:
            raise RuntimeError(f"V175 dependency drifted: {key}")
    ...
    if safety_passed and beneficial and strong_development:
        ...
        decision = config["decisionRule"]["ifSafetyAndBenefitPassButStrongFails"]
    elif safety_passed:
    ...
    result = {
        "schema_version": "175-certification-aware-planner-development-result",
