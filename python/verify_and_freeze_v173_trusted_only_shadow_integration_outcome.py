...
from run_v173_trusted_only_shadow_integration import DEPENDENCY_KEYS, reconstruct
...
def main() -> None:
    ...
    result_path = PROJECT_ROOT / "outputs/v173-trusted-only-shadow-integration/evaluation/result.json"
    doc_path = PROJECT_ROOT / "docs/v173-trusted-only-shadow-integration-results.md"
    verifier_path = PROJECT_ROOT / "python/verify_and_freeze_v173_trusted_only_shadow_integration_outcome.py"
    audit_path = PROJECT_ROOT / "outputs/v173-trusted-only-shadow-integration/outcome-audit.json"
    outcome_path = PROJECT_ROOT / "configs/v173-trusted-only-shadow-integration-outcome-lock.json"
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V173 outcome is already frozen")
    if not doc_path.is_file():
        raise RuntimeError("write V173 results before freezing")

    lock = json.loads(lock_path.read_text())
    result = json.loads(result_path.read_text())
    evaluation = reconstruct(lock)
    config = lock["config_payload"]
    safety = evaluate_integrity_and_safety_gates(evaluation, result["access"], config)
    ...
    if safety_passed and benefit_passed and strong_passed:
        expected_decision = config["decisionRule"]["ifSafetyBenefitAndStrongThresholdsPass"]
    elif safety_passed and benefit_passed:
        expected_decision = config["decisionRule"]["ifSafetyAndBenefitPassButStrongThresholdsFail"]
    elif safety_passed:
    ...
    expected = {
        "state_policy_results": {"state_policy_results": evaluation["state_policy_results"], "target_cases_subsampled": False},
        "integration_summary": evaluation["summary"],
    ...
    checks = {
        ...
        "all_persisted_and_digest_outputs_reconstruct_exactly": outputs_exact,
        "summary_gates_thresholds_and_decision_reconstruct_exactly": bool(
            result["summary"] == evaluation["summary"]
            and result["integrity_and_safety_gates"] == safety
            and result["benefit_thresholds"] == benefit
            and result["strong_integration_thresholds"] == strong
            and result["passed"] == safety_passed
            and result["beneficial"] == benefit_passed
            and result["strong_integration"] == strong_passed
            and result["decision"] == expected_decision
        ),
        "target_policy_population_is_complete_and_source_risks_reconstruct": bool(
            evaluation["summary"]["eligible_state_count"] == 132
            and evaluation["summary"]["target_case_count"] == 4224
            and evaluation["summary"]["target_policy_score_count"] == 29568
            and evaluation["summary"]["source_V167_recommendation_risk_reconstruction_rate"] == 1.0
        ),
        "model_provisional_real_effect_and_execution_boundary_holds": all(
            result["access"][key] == 0
            for key in (
    ...
    audit = {
        "schema_version": "173-trusted-only-shadow-integration-outcome-audit",
        "experiment": config["experiment"],
        "passed": passed,
        "scientific_safety_gates_passed": result["passed"],
        "beneficial": result["beneficial"],
        "strong_integration": result["strong_integration"],
        "decision": "freeze_verified_V173_outcome" if passed else "reject_V173_outcome",
        "checks": checks,
        "independent_summary": evaluation["summary"],
        "additional_access": {"model_load_count": 0, "real_service_call_count": 0, "actual_execution_count": 0},
    }
    ...
    dependencies = {
        ...
        "parent_V172_outcome": PROJECT_ROOT / lock["parent_V172_outcome"],
        "source_V170_outcome": PROJECT_ROOT / lock["source_V170_outcome"],
    ...
    outcome: dict[str, Any] = {
        "schema_version": "173-trusted-only-shadow-integration-outcome-lock",
        "experiment": config["experiment"],
        "outcome": {
            "passed": True,
            "scientific_safety_gates_passed": result["passed"],
        ...
        "authorization": {
            "modify_rerun_select_subsample_or_tune_V173": False,
            "retain_outcome_without_posthoc_selection": True,
            "design_nonoverlapping_integration_confirmation": bool(result["beneficial"]),
