...
def main() -> None:
    ...
    if args.manifest.exists() or args.snapshot_directory.exists():
        raise RuntimeError("V215 metadata capture output already exists")
    ...
    for unit in config["sourceUnits"]:
        for url in unit["urls"]:
            ...
            snapshot = args.snapshot_directory / f"{key}.html"
            row = {
                "unit_id": unit["unitId"],
                "url": url,
                "success": False,
            ...
            attempts.append(row)
    manifest = {
        "schema_version": "215-official-metadata-retrieval-manifest",
