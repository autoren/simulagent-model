...
def main() -> None:
    source_lock_path = PROJECT_ROOT / "configs/v95-activation-open-set-source-lock.json"
    result_path = PROJECT_ROOT / "outputs/v95-activation-open-set/source-inventory/result.json"
    inventory_path = PROJECT_ROOT / "outputs/v95-activation-open-set/source-inventory/activation-open-set-inventory.json"
    doc_path = PROJECT_ROOT / "docs/v95-activation-open-set-source-results.md"
    verifier_path = PROJECT_ROOT / "python/verify_and_freeze_v95_source_outcome.py"
    audit_path = PROJECT_ROOT / "outputs/v95-activation-open-set/source-outcome-audit.json"
    outcome_path = PROJECT_ROOT / "configs/v95-activation-open-set-source-outcome-lock.json"
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V95 source outcome is already frozen")
    if not doc_path.is_file():
        raise RuntimeError("write the V95 source result before freezing")
    ...
    reconstructed = build_activation_open_set_inventory(
    ...
    checks = {
        ...
        "partition_sets_are_disjoint_and_service_stratified": bool(
        ...
        "result_and_gate_decision_are_consistent": bool(
            ...
            and result["decision"] == (
                "freeze_source_and_preregister_activation_open_set_population"
                if result["passed"] else "stop_V95_before_population_or_model_access"
            )
        ),
    ...
    audit = {
        "schema_version": "95-activation-open-set-source-outcome-audit",
        "experiment": "v95_activation_open_set_source_outcome_audit",
        "passed": integrity_passed,
        "scientific_source_feasibility_passed": result["passed"],
        "decision": (
            "freeze_positive_V95_activation_source_feasibility" if result["passed"]
            else "freeze_negative_V95_activation_source_feasibility"
        ) if integrity_passed else "reject_V95_source_outcome",
        "checks": checks,
        "independent_summary": {
    ...
    outcome: dict[str, Any] = {
        "schema_version": "95-activation-open-set-source-outcome-lock",
        "experiment": "v95_activation_open_set_source_outcome_lock",
        "outcome": {
            "passed": True,
            "scientific_source_feasibility_passed": result["passed"],
        ...
        "authorization": {
            "modify_or_rerun_V95_source_stage": False,
