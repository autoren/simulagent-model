...
STAGES = (
    "request_lexical_control", "request_uncatalogued_paraphrase", "request_relational_tie",
    "request_insufficient", "closed_route_family", "closed_route_unclear",
    "closed_specific_left", "closed_specific_right",
)
...
def build_catalog(config: dict[str, Any]) -> dict[str, Any]:
    ...
    generic_options = [
        ...
        "text": generic["unclear_option_text"], "route_query_id": None,
    }]
    queries = [{
        "query_id": generic["query_id"], "title": generic["title"],
        "question": generic["question"], "query_kind": "GENERIC_ROUTE",
        "options": generic_options,
    }]
    for family in config["families"]:
        queries.append({
            "query_id": family["query_id"], "title": family["query_title"],
            "question": family["question"], "query_kind": "SPECIFIC_WITNESS",
    ...
    payload = {
        "schema_version": "157-hard-tie-generic-routing-interaction-catalog",
        "choices": deepcopy(config["catalog"]["choices"]), "queries": queries,
    }
    payload["catalog_sha256"] = payload_hash(payload)
    return payload


def _find_option(event: Any, catalog: dict[str, Any]) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
...
def malformed_answer_events(catalog: dict[str, Any]) -> list[Any]:
    ...
    return [
        None, {}, {"query_id": generic["query_id"]}, {"selected_option_id": generic["options"][0]["option_id"]},
        {"query_id": "UNKNOWN", "selected_option_id": generic["options"][0]["option_id"]},
        {"query_id": generic["query_id"], "selected_option_id": "UNKNOWN"},
        {"query_id": generic["query_id"], "selected_option_id": generic["options"][0]["option_id"], "extra": True},
...
def _fixture_id(group_id: str, stage: str) -> str:
    return "v157-" + hashlib.sha256(f"{group_id}|{stage}".encode()).hexdigest()[:16]
...
def build_population(config: dict[str, Any]) -> dict[str, Any]:
    ...
    for family_index, family in enumerate(config["families"]):
        ...
        decoy_pair = [decoy["left_choice_id"], decoy["right_choice_id"]]
        for variant_index, slots in enumerate(_slot_variants(family)):
            split = "development" if variant_index < 4 else "evaluation"
            group_id = f"v157-g{family_index:02d}-{variant_index:02d}"
            render = lambda key: family[key].format(**slots)
            lexical = render("lexical_control")
            uncatalogued = render("uncatalogued_paraphrase")
            relational = render("relational_tie")
            insufficient = render("insufficient")
            route_answer = render("route_answer")
            specs = {
                "request_lexical_control": (
                    lexical, left, [left], None, "lexical_control", family["query_id"], family["query_id"]
                ),
                "request_uncatalogued_paraphrase": (
                    uncatalogued, right, [right], None, "uncatalogued_paraphrase", family["query_id"], config["genericQueryId"]
                ),
                "request_relational_tie": (
                    relational, config["insufficientId"], sorted(set([left, right, *decoy_pair])), None,
                    "relational_tie", family["query_id"], config["genericQueryId"]
                ),
                "request_insufficient": (
                    insufficient, config["insufficientId"], list(config["outputIds"]), None,
                    "insufficient", None, config["genericQueryId"]
                ),
                "closed_route_family": (
                    route_answer, config["insufficientId"], sorted([left, right]),
                    {"query_id": config["genericQueryId"], "selected_option_id": f"ROUTE_{family['query_id']}"},
                    "trusted_route_family", family["query_id"], None
                ),
                "closed_route_unclear": (
                    "The service area still cannot be identified.", config["insufficientId"], list(config["outputIds"]),
                    {"query_id": config["genericQueryId"], "selected_option_id": config["genericUnclearOptionId"]},
                    "trusted_route_unclear", None, None
                ),
                "closed_specific_left": (
                    render("left_answer"), left, [left],
                    {"query_id": family["query_id"], "selected_option_id": "LEFT"},
                    "trusted_specific_left", family["query_id"], None
                ),
                "closed_specific_right": (
                    render("right_answer"), right, [right],
                    {"query_id": family["query_id"], "selected_option_id": "RIGHT"},
                    "trusted_specific_right", family["query_id"], None
                ),
            }
            for stage in STAGES:
                text, truth, compatible, event, stratum, route_target, oracle_initial = specs[stage]
                if stage in REQUEST_STAGES:
                    conversation = [{"role": "user", "text": text}]
                elif stage == "closed_route_family":
                    conversation = [
                        {"role": "user", "text": relational},
                        {"role": "assistant", "text": query_by_id[config["genericQueryId"]]["question"]},
                        {"role": "user", "text": text},
                    ]
                elif stage == "closed_route_unclear":
                    conversation = [
                        {"role": "user", "text": insufficient},
                        {"role": "assistant", "text": query_by_id[config["genericQueryId"]]["question"]},
                        {"role": "user", "text": text},
                    ]
                else:
                ...
                hidden = {
                    **public, "group_id": group_id, "family_id": family["family_id"],
                    "stage": stage, "stratum": stratum, "truth_state_id": truth,
                    ...
                    "oracle_witness": witness, "variant_index": variant_index,
                }
    ...
    summary = {
        ...
        "group_count": len({row["group_id"] for row in hidden_rows}), "fixture_count": len(hidden_rows),
...
def audit_population(
    ...
    forbidden = {
        "group_id", "family_id", "stage", "stratum", "truth_state_id", "compatible_state_ids",
        "oracle_specific_query_id", "oracle_initial_query_id", "route_target_query_id",
        "oracle_route", "oracle_witness", "variant_index",
    }
    ...
    malformed_routes = [route_from_answer_event(event, catalog) for event in malformed_answer_events(catalog)]
    ...
    checks = {
        "choice_query_and_generic_route_contract": bool(
            len(catalog["choices"]) == 9
            and kinds == Counter({"KNOWN": 6, "NOVEL_CANDIDATE": 1, "UNSUPPORTED": 1, "INSUFFICIENT_EVIDENCE": 1})
            and len(catalog["queries"]) == 7 and len(specific_queries) == 6
            and generic["query_id"] == config["genericQueryId"] and len(generic["options"]) == 7
        ...
        "population_split_stage_and_stratum_counts": bool(
            summary["group_count"] == 48 and summary["fixture_count"] == 384
            and summary["request_fixture_count"] == 192 and summary["route_fixture_count"] == 96
            and summary["specific_answer_fixture_count"] == 96
            and summary["split_counts"] == {"development": 192, "evaluation": 192}
            and all(count == 48 for count in summary["stage_counts"].values())
            and all(count == 48 for count in summary["stratum_counts"].values())
        ),
        "group_stage_completeness": all(
            len(rows) == 8 and {row["stage"] for row in rows} == set(STAGES)
            and len({row["split"] for row in rows}) == 1 for rows in groups.values()
        ),
        "public_hidden_alignment_without_truth_route_or_oracle_leakage": bool(
        ...
        "generic_route_events_valid_without_semantic_witness": bool(
            ...
            and all(
                (row["stage"] == "closed_route_family" and route_records[index]["target_query_id"] == row["route_target_query_id"])
                or (row["stage"] == "closed_route_unclear" and route_records[index]["target_query_id"] is None)
    ...
    return {
        ...
        "policy_score_count": 0, "model_load_count": 0, "model_generation_or_score_count": 0,
