...
def main() -> None:
    ...
    output_dir = PROJECT_ROOT / "outputs/v160-model-free-controlled-relational-grammar-policy/model-free-realization"
    result_path = output_dir / "result.json"
    access_path = output_dir / "access.json"
    if output_dir.exists():
        raise RuntimeError("V160 realization already exists")
    ...
    if not valid_lock(lock) or not all(
        ...
        raise RuntimeError("V160 lock or dependencies changed")
    ...
    access = {
        ...
        "policy_request_decision_count": len(public_requests) * 2,
        "policy_specific_query_score_count": len(public_requests) * 4 + 32 * 4,
        "grammar_alias_lookup_request_count": 32,
        "unit_contract_probe_count_before_lock": 1,
        ...
        json.dumps(
            {
                ...
                "comparator_metrics": result["comparator_metrics"],
                "access": access,
            },
            indent=2,
            sort_keys=True,
