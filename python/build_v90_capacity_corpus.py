#!/usr/bin/env python3
"""Select, extract, and seal the fresh V90 human-language population exactly once."""
...
def structurally_select(config: dict[str, Any], rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for stratum in config["population"]["strata"]:
        ...
        for row in candidates:
            ...
            if len(chosen) == stratum["count"]:
                break
        if len(chosen) != stratum["count"]:
            raise RuntimeError(f"underfilled unique-dialogue V90 stratum: {stratum}")
...
def main() -> None:
    ...
    corpus_path = PROJECT_ROOT / "data/v90-capacity-generation/records.jsonl"
    seal_path = PROJECT_ROOT / "data/v90-capacity-generation/corpus-seal.json"
    if corpus_path.exists() or seal_path.exists():
        raise RuntimeError("V90 corpus is already materialized or sealed")
    ...
    if payload_hash(design_payload) != design["lock_payload_sha256"]:
        raise RuntimeError("V90 design lock payload mismatch")
    if not design["authorization"]["select_extract_and_seal_corpus_once"]:
        raise RuntimeError("V90 design does not authorize corpus construction")
    for key in ("design", "source_outcome_lock", "source_inventory", "plan", "auditor", "builder"):
    ...
    schema_lock = json.loads((PROJECT_ROOT / "configs/v87-external-language-source-outcome-lock.json").read_text())
    schema_path = PROJECT_ROOT / schema_lock["source_files"]["dev/schema.json"]["local_path"]
    if file_sha256(source_file) != source_lock["source_file_sha256"]:
        raise RuntimeError("V90 source shard drifted")
    if file_sha256(schema_path) != schema_lock["source_files"]["dev/schema.json"]["local_sha256"]:
        raise RuntimeError("V90 schema dependency drifted")
    ...
    structural_by_id = {item["record_id"]: item for item in inventory["record_index"]}
    selected = structurally_select(config, inventory["record_index"])
    if len(selected) != 48 or len({row["dialogue_id"] for row in selected}) != 48:
        raise RuntimeError("V90 selected population cardinality or dialogue uniqueness drifted")

    records = []
    salt = config["population"]["selectionSalt"]
    for sequence, structural in enumerate(selected):
        if structural_by_id[structural["record_id"]] != structural:
            raise RuntimeError("V90 structural identity mismatch")
        ...
        if turn["speaker"] != "USER" or len(turn["frames"]) != 1:
            raise RuntimeError("V90 selected turn violates frozen structure")
        frame = turn["frames"][0]
        state = frame["state"]
        if frame["service"] != structural["service"] or state["active_intent"] != structural["active_intent"]:
            raise RuntimeError("V90 source target drifted")
        if sorted(state["slot_values"]) != structural["state_slot_keys"]:
            raise RuntimeError("V90 source state-key target drifted")
        service_schema = schema_by_service[structural["service"]]
        allowed_intents = [item["name"] for item in service_schema["intents"]]
        allowed_slots = [item["name"] for item in service_schema["slots"]]
        ...
        records.append({
            "id": f"v90-{sequence:03d}-{structural['record_id']}",
            ...
            "schema_context": {
                ...
                "intents": [{"id": item["name"], "description": item["description"]} for item in service_schema["intents"]],
                "slots": [{"id": item["name"], "description": item["description"]} for item in service_schema["slots"]],
            },
            "allowed_intent_ids": allowed_intents + ["NONE"],
    ...
    seal = {
        "schema_version": "90-capacity-generation-corpus-seal",
        "experiment": "v90_capacity_generation_corpus_seal",
        "design_lock": str(design_path.relative_to(PROJECT_ROOT)),
        "design_lock_sha256": file_sha256(design_path),
        "source_outcome_lock": design["source_outcome_lock"],
        "source_outcome_lock_sha256": file_sha256(PROJECT_ROOT / design["source_outcome_lock"]),
        ...
        "stratum_counts": {f"{k[0]}::{k[1]}": v for k, v in sorted(strata.items())},
        ...
        "authorization": {
            ...
            "implement_and_audit_model_acquisition_and_runner": True,
            "download_pinned_model_weights_after_acquisition_code_lock": False,
