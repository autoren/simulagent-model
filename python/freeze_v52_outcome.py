...
def main():
    ...
    parser.add_argument("--output", default="configs/v52-outcome-lock.json")
    ...
    if output.exists():
        raise RuntimeError("V52 outcome already frozen")
    result = json.loads(result_path.read_text())
    audit = json.loads(audit_path.read_text())
    if not audit["passed"] or audit["result_sha256"] != file_sha256(result_path):
        raise RuntimeError("V52 post-result audit is not bound to result")
    lock = {
        "schema_version": 52,
        "experiment": "v52_outcome_lock",
