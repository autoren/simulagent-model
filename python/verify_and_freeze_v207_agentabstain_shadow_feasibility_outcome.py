...
from v207_agentabstain_shadow_feasibility import audit_feasibility, evaluate_feasibility
...
def main() -> None:
    ...
    audit_path = PROJECT_ROOT / "outputs/v207-agentabstain-shadow-feasibility/outcome-audit.json"
    outcome_path = PROJECT_ROOT / "configs/v207-agentabstain-shadow-feasibility-outcome-lock.json"
    results_path = PROJECT_ROOT / "docs/v207-agentabstain-shadow-feasibility-results.md"
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V207 outcome already frozen")
    ...
    rebuilt = evaluate_feasibility(
        lock["config_payload"], dataset_commit=stored_summary["dataset"]["commit"]
    )
    ...
    expected_decision = lock["config_payload"]["decisionRule"][
        "ifEveryIntegrityQualificationAndAccessGatePasses" if scientific_pass else "otherwise"
    ]
    result_exact = bool(
    ...
    checks = {
        ...
        "pinned_metadata_and_schema_reconstruct_exactly": summary_exact,
    ...
    outcome_audit = {
        "schema_version": "207-agentabstain-shadow-metadata-schema-feasibility-outcome-audit",
        "experiment": lock["experiment"],
        "passed": passed,
        "scientific_feasibility_passed": scientific_pass,
        "decision": "freeze_verified_V207_feasibility" if passed else "freeze_failed_V207_verification",
        "checks": checks,
        "summary": rebuilt,
    }
    ...
    outcome: dict[str, Any] = {
        "schema_version": "207-agentabstain-shadow-metadata-schema-feasibility-outcome-lock",
        "experiment": lock["experiment"],
        "outcome": {
            "passed": True,
            "scientific_feasibility_passed": scientific_pass,
        ...
        "authorization": {
            "preregister_separate_deterministic_text_extraction_only": scientific_pass,
