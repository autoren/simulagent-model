from __future__ import annotations

from collections import Counter
from io import BytesIO
import json
from pathlib import PurePosixPath
import re
import unicodedata
from typing import Any, Iterable
import zipfile

from v93_open_set_source import canonical_sha256


TARGET_ARGUMENT_PATTERN = re.compile(r"«\s*(.*?)\s*»")
TARGET_ROOT_PATTERN = re.compile(r"^\s*([A-Za-z][A-Za-z0-9_]*)\s*\(")


def normalized_tokens(value: str) -> tuple[str, ...]:
    if not isinstance(value, str):
        raise ValueError("language value must be a string")
    normalized = unicodedata.normalize("NFKC", value).casefold()
    return tuple(re.findall(r"[^\W_]+", normalized, flags=re.UNICODE))


def contains_phrase(container: str, phrase: str) -> bool:
    container_tokens = normalized_tokens(container)
    phrase_tokens = normalized_tokens(phrase)
    if not phrase_tokens or len(phrase_tokens) > len(container_tokens):
        return False
    width = len(phrase_tokens)
    return any(
        container_tokens[index:index + width] == phrase_tokens
        for index in range(len(container_tokens) - width + 1)
    )


def target_arguments(target: str, rule: dict[str, Any]) -> tuple[str, ...]:
    prohibited = frozenset(rule["prohibitedNormalizedArguments"])
    values = []
    for match in TARGET_ARGUMENT_PATTERN.finditer(target):
        value = match.group(1).strip()
        tokens = normalized_tokens(value)
        normalized = " ".join(tokens)
        if (
            len(normalized.replace(" ", "")) >= rule["minimumNormalizedArgumentCharacterCount"]
            and len(tokens) <= rule["maximumNormalizedArgumentTokenCount"]
            and normalized not in prohibited
        ):
            values.append(value)
    return tuple(values)


def context_surfaces(metadata: dict[str, Any]) -> list[tuple[str, str]]:
    surfaces: list[tuple[str, str]] = []
    previous_turns = metadata["previous_turns"]
    seeded_lists = metadata["seeded_lists"]
    seeded_notes = metadata["seeded_notes"]
    seeded_contacts = metadata["seeded_contacts"]
    if not isinstance(previous_turns, list):
        raise ValueError("previous_turns must be a list")
    for turn in previous_turns:
        if not isinstance(turn, dict):
            raise ValueError("previous turn must be an object")
        for field, kind in (
            ("user_query", "previous_turn_user_query"),
            ("response_text", "previous_turn_response_text"),
        ):
            value = turn.get(field)
            if not isinstance(value, str):
                raise ValueError(f"previous turn {field} must be a string")
            surfaces.append((kind, value))
    if not isinstance(seeded_lists, list):
        raise ValueError("seeded_lists must be a list")
    for item in seeded_lists:
        if not isinstance(item, dict) or not isinstance(item.get("name"), str):
            raise ValueError("seeded list is invalid")
        surfaces.append(("seeded_list_name", item["name"]))
        if not isinstance(item.get("items"), list) or not all(
            isinstance(value, str) for value in item["items"]
        ):
            raise ValueError("seeded list items are invalid")
        surfaces.extend(("seeded_list_item", value) for value in item["items"])
    if not isinstance(seeded_notes, list):
        raise ValueError("seeded_notes must be a list")
    for note in seeded_notes:
        if (
            not isinstance(note, dict)
            or not isinstance(note.get("name"), str)
            or not isinstance(note.get("text"), str)
        ):
            raise ValueError("seeded note is invalid")
        surfaces.append(("seeded_note_name", note["name"]))
        surfaces.append(("seeded_note_text", note["text"]))
    if not isinstance(seeded_contacts, list) or not all(
        isinstance(value, str) for value in seeded_contacts
    ):
        raise ValueError("seeded_contacts must be a string list")
    surfaces.extend(("seeded_contact", value) for value in seeded_contacts)
    return surfaces


def parse_presto_archive(
    data: bytes, required_basenames: Iterable[str]
) -> tuple[list[tuple[str, dict[str, Any]]], dict[str, str]]:
    required = tuple(required_basenames)
    with zipfile.ZipFile(BytesIO(data)) as archive:
        members: dict[str, str] = {}
        for basename in required:
            matches = [
                name for name in archive.namelist()
                if not name.endswith("/") and PurePosixPath(name).name == basename
            ]
            if len(matches) != 1:
                raise ValueError(f"expected one {basename} member, found {len(matches)}")
            members[basename] = matches[0]
        records: list[tuple[str, dict[str, Any]]] = []
        for basename in required:
            with archive.open(members[basename]) as handle:
                for line_number, line in enumerate(handle, start=1):
                    try:
                        value = json.loads(line)
                    except json.JSONDecodeError as error:
                        raise ValueError(f"invalid {basename} JSONL line {line_number}") from error
                    if not isinstance(value, dict):
                        raise ValueError(f"{basename} line {line_number} is not an object")
                    records.append((basename, value))
    return records, members


def build_presto_context_inventory(
    source_records: Any, config: dict[str, Any]
) -> dict[str, Any]:
    if not isinstance(source_records, list) or not source_records:
        raise ValueError("PRESTO records must be a non-empty list")
    required_record = frozenset(config["requiredRecordFields"])
    required_metadata = frozenset(config["requiredMetadataFields"])
    allowed_splits = frozenset(config["allowedSourceSplits"])
    split_map = config["canonicalSplitMap"]
    rule = config["dependencyRule"]
    eligible_sources = frozenset(rule["eligibleContextSources"])
    seen: set[str] = set()
    candidates = []
    member_counts: Counter[str] = Counter()
    split_counts: Counter[str] = Counter()
    en_us_human_counts: Counter[str] = Counter()
    semantic_roots: set[str] = set()
    synthetic_context_candidate_count = 0
    for entry in source_records:
        if not isinstance(entry, tuple) or len(entry) != 2:
            raise ValueError("PRESTO source entry must contain member and record")
        member, row = entry
        if not isinstance(member, str) or not isinstance(row, dict) or not required_record <= row.keys():
            raise ValueError("PRESTO record lacks required fields")
        inputs = row["inputs"]
        target = row["targets"]
        metadata = row["metadata"]
        if not isinstance(inputs, str) or not isinstance(target, str) or not isinstance(metadata, dict):
            raise ValueError("PRESTO input, target, or metadata is invalid")
        if not required_metadata <= metadata.keys():
            raise ValueError("PRESTO metadata lacks required fields")
        identifier = str(metadata["example_id"])
        source_split = metadata["split"]
        if not identifier or identifier in seen or source_split not in allowed_splits:
            raise ValueError("PRESTO identifier or split is invalid")
        expected_member = f"presto_{source_split}.jsonl"
        if member != expected_member:
            raise ValueError("PRESTO member and metadata split disagree")
        seen.add(identifier)
        member_counts[member] += 1
        split_counts[source_split] += 1
        if metadata["locale"] != config["locale"]:
            continue
        if metadata["context"] != config["requiredContextProvenance"]:
            continue
        en_us_human_counts[source_split] += 1
        surfaces = context_surfaces(metadata)
        matches: list[tuple[str, str]] = []
        for argument in target_arguments(target, rule):
            if contains_phrase(inputs, argument):
                continue
            for source_kind, surface in surfaces:
                if source_kind in eligible_sources and contains_phrase(surface, argument):
                    matches.append((argument, source_kind))
        if not matches:
            continue
        source_kinds = sorted({kind for _, kind in matches})
        root_match = TARGET_ROOT_PATTERN.match(target)
        if root_match:
            semantic_roots.add(root_match.group(1))
        if metadata["context"] == "synthetic":
            synthetic_context_candidate_count += 1
        role = split_map[source_split]
        candidates.append({
            "candidate_id": f"presto::{identifier}",
            "source_id": identifier,
            "source_member": member,
            "source_split": source_split,
            "role": role,
            "dependency_source_kinds": source_kinds,
            "dependency_argument_count": len({" ".join(normalized_tokens(arg)) for arg, _ in matches}),
            "previous_turn_count": len(metadata["previous_turns"]),
            "seeded_list_count": len(metadata["seeded_lists"]),
            "seeded_note_count": len(metadata["seeded_notes"]),
            "seeded_contact_count": len(metadata["seeded_contacts"]),
            "full_context_pair_id": f"presto::{identifier}::full",
            "ablated_context_pair_id": f"presto::{identifier}::ablated",
        })
    candidates.sort(key=lambda row: row["candidate_id"])
    forbidden = {
        "inputs", "targets", "input", "target", "argument", "context_text", "tokens",
        "seeded_values", "utterance", "text", "prompt",
    }
    keys = set().union(*(row.keys() for row in candidates)) if candidates else set()
    if keys & forbidden:
        raise AssertionError("PRESTO language leaked into inventory")
    role_counts = Counter(row["role"] for row in candidates)
    dependency_counts = Counter(
        kind for row in candidates for kind in row["dependency_source_kinds"]
    )
    previous_turn_kinds = {"previous_turn_user_query", "previous_turn_response_text"}
    seeded_kinds = eligible_sources - previous_turn_kinds
    development_ids = {
        row["source_id"] for row in candidates if row["role"] == "development"
    }
    test_ids = {
        row["source_id"] for row in candidates if row["role"] == "protected_test"
    }
    return {
        "source_record_count": len(source_records),
        "source_member_record_counts": dict(sorted(member_counts.items())),
        "source_split_record_counts": dict(sorted(split_counts.items())),
        "en_us_human_context_record_counts": dict(sorted(en_us_human_counts.items())),
        "eligible_candidate_count": len(candidates),
        "role_candidate_counts": dict(sorted(role_counts.items())),
        "dependency_source_kind_counts": dict(sorted(dependency_counts.items())),
        "dependency_source_kind_count": len(dependency_counts),
        "previous_turn_dependent_candidate_count": sum(
            bool(set(row["dependency_source_kinds"]) & previous_turn_kinds) for row in candidates
        ),
        "seeded_state_dependent_candidate_count": sum(
            bool(set(row["dependency_source_kinds"]) & seeded_kinds) for row in candidates
        ),
        "semantic_root_function_count": len(semantic_roots),
        "synthetic_context_candidate_count": synthetic_context_candidate_count,
        "development_test_identifiers_are_disjoint": not (development_ids & test_ids),
        "candidate_index_sha256": canonical_sha256(candidates),
        "candidate_index": candidates,
        "pairs_share_source_id_input_and_target_by_construction": True,
        "contains_input_target_argument_context_tokens_seeded_values_or_prompts": False,
    }


def evaluate_presto_source_gates(
    inventory: dict[str, Any], config: dict[str, Any]
) -> dict[str, bool]:
    gates = config["sourceGates"]
    return {
        "development_candidate_count": (
            inventory["role_candidate_counts"].get("development", 0)
            >= gates["minimumEligibleDevelopmentCandidateCount"]
        ),
        "protected_test_candidate_count": (
            inventory["role_candidate_counts"].get("protected_test", 0)
            >= gates["minimumEligibleProtectedTestCandidateCount"]
        ),
        "total_candidate_count": (
            inventory["eligible_candidate_count"] >= gates["minimumEligibleTotalCandidateCount"]
        ),
        "previous_turn_dependency_count": (
            inventory["previous_turn_dependent_candidate_count"]
            >= gates["minimumPreviousTurnDependentCandidateCount"]
        ),
        "seeded_state_dependency_count": (
            inventory["seeded_state_dependent_candidate_count"]
            >= gates["minimumSeededStateDependentCandidateCount"]
        ),
        "dependency_source_kind_count": (
            inventory["dependency_source_kind_count"] >= gates["minimumDependencySourceKindCount"]
        ),
        "semantic_root_function_count": (
            inventory["semantic_root_function_count"] >= gates["minimumSemanticRootFunctionCount"]
        ),
        "development_test_identifier_disjointness": bool(
            inventory["development_test_identifiers_are_disjoint"]
        ),
        "zero_synthetic_context_candidates": (
            inventory["synthetic_context_candidate_count"]
            <= gates["maximumSyntheticContextCandidateCount"]
        ),
        "paired_identity_boundary": bool(
            inventory["pairs_share_source_id_input_and_target_by_construction"]
        ),
        "text_free_inventory": not (
            inventory["contains_input_target_argument_context_tokens_seeded_values_or_prompts"]
        ),
    }


__all__ = [
    "build_presto_context_inventory", "contains_phrase", "context_surfaces",
    "evaluate_presto_source_gates", "normalized_tokens", "parse_presto_archive",
    "target_arguments",
]
