...
SLOT_VALUE_PATTERN = re.compile(r"\[\s*([^\[\]:]+?)\s*:\s*([^\[\]]*?)\s*\]")


def parse_annotated_slots(value: str) -> list[dict[str, str]]:
    if not isinstance(value, str):
        raise ValueError("annotated utterance must be a string")
    return [
        {"slot_type": match.group(1).strip(), "slot_value": match.group(2).strip()}
        for match in SLOT_VALUE_PATTERN.finditer(value)
    ]
...
def build_selected_language_artifacts(
    ...
    for selected_row in selected:
        ...
        if str(source["id"]) != selected_row["source_id"]:
            raise ValueError("selected and source identifiers disagree")
        structure_fields = {
        ...
        exact_structure = exact_structure and bool(
            candidate["source_id"] == structure_fields["source_id"]
            and candidate["partition"] == selected_row["source_partition"]
            and config["canonicalSourcePartitionMap"][structure_fields["partition"]]
            == selected_row["source_partition"]
            and candidate["scenario"] == selected_row["scenario"] == structure_fields["scenario"]
            and candidate["intent"] == selected_row["intent"] == structure_fields["intent"]
            and candidate["class_label"] == selected_row["class_label"]
        )
        computed_overlap = len(normalized_tokens(source["utt"]) & normalized_tokens(source["intent"]))
        if selected_row["class_label"] == "known_familiar":
            ...
            exact_familiarity = exact_familiarity and bool(
                computed_overlap == selected_row["current_utterance_intent_overlap_count"] == 0
            )
        ...
        role_records[selected_row["role"]].append({
    ...
    all_records = [row for role in sorted(role_records) for row in role_records[role]]
    ...
    return {
        ...
        "exact_selected_identifier_set": emitted_ids == selected_ids,
        ...
        "development_protected_test_disjoint": not (
            {row["candidate_id"] for row in role_records.get("development", [])}
            & {row["candidate_id"] for row in role_records.get("protected_test", [])}
        ),
        "unselected_language_record_count": 0,
        "role_payload_sha256": {
            role: canonical_sha256(records) for role, records in sorted(role_records.items())
        },
...
def evaluate_extraction_gates(
    ...
    checks = {
        "total_record_count": artifacts["total_record_count"] == gates["requiredTotalRecordCount"],
        ...
        "development_protected_test_disjointness": artifacts["development_protected_test_disjoint"],
        "zero_unselected_language_records": (
            artifacts["unselected_language_record_count"] <= gates["maximumUnselectedLanguageRecordCount"]
        ),
    }
    for role in config["roles"]:
        checks[f"{role}_record_count"] = (
            artifacts["role_record_counts"].get(role, 0) == gates["requiredRecordCountPerRole"]
        )
...
__all__ = [
    "build_selected_language_artifacts", "evaluate_extraction_gates", "parse_annotated_slots",
]
