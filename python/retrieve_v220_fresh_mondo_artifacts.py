...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: retrieve_v220_fresh_mondo_artifacts.py LOCK")
    ...
    if not lock["authorization"]["retrieve_exactly_eight_frozen_payloads_once"]:
        raise RuntimeError("V220 payload retrieval is not authorized")
    ...
    if manifest_path.exists() or any((PROJECT_ROOT / row["rawPath"]).exists() for row in config["payloads"]):
        raise RuntimeError("V220 retrieval output already exists")
    ...
    for payload in config["payloads"]:
        ...
        attempts.append(row)
    manifest = {
        "schema_version": "220-fresh-mondo-artifact-retrieval-manifest",
        ...
        "release_summary_network_request_count": 0,
    }
