#!/usr/bin/env python3
"""Download the pinned untouched SGD shard once and emit only a structural inventory."""
...
def main() -> None:
    ...
    output_root = PROJECT_ROOT / "outputs/v90-capacity-generation"
    source_root = output_root / "source"
    inventory_root = output_root / "source-inventory"
    if source_root.exists() or inventory_root.exists():
        raise RuntimeError("V90 source acquisition and inventory may run only once")
    ...
    if payload_hash(lock_payload) != lock["lock_payload_sha256"]:
        raise RuntimeError("V90 source lock payload mismatch")
    for key in ("config", "parent_outcome_lock", "plan", "inventory_module", "runner", "verifier", "auditor"):
    ...
    if not lock["authorization"]["download_and_inventory_pinned_shard_once"]:
        raise RuntimeError("V90 source acquisition is not authorized")

    config = lock["config_payload"]
    shard = config["newDialogueShard"]
    schema_spec = config["schemaDependency"]
    request = Request(shard["rawUrl"], headers={"User-Agent": "simulagent-v90-source-extension"})
    with urlopen(request, timeout=60) as response:  # noqa: S310 - exact immutable HTTPS URL is locked
        dialogue_bytes = response.read(shard["byteSize"] + 1)
    if len(dialogue_bytes) != shard["byteSize"]:
        raise RuntimeError("V90 source shard byte size mismatch")
    if git_blob_sha1(dialogue_bytes) != shard["gitBlobSha1"]:
        raise RuntimeError("V90 source shard Git blob mismatch")

    schema_path = PROJECT_ROOT / schema_spec["localPath"]
    if file_sha256(schema_path) != schema_spec["localSha256"]:
        raise RuntimeError("V90 pinned schema dependency drifted")
    ...
    inventory = build_structural_inventory(
        schema_payload,
        dialogue_payload,
        excluded_service_prefixes=tuple(config["inventoryProtocol"]["excludedServicePrefixes"]),
    )
    inventory_artifact = {
        "provenance": {
    ...
    active_intents = {
        key for key in inventory["eligible_intent_counts"] if not key.endswith("::NONE")
    }
    gate_checks = {
        "eligible_record_count": counts["eligible_record_count"] >= gates["minimumEligibleRecordCount"],
        "eligible_active_record_count": counts["eligible_active_record_count"] >= gates["minimumEligibleActiveRecordCount"],
        "eligible_none_record_count": counts["eligible_none_record_count"] >= gates["minimumEligibleNoneRecordCount"],
        "eligible_service_count": len(inventory["eligible_service_counts"]) >= gates["minimumEligibleServiceCount"],
        "eligible_active_intent_count": len(active_intents) >= gates["minimumEligibleActiveIntentCount"],
    ...
    result = {
        "schema_version": "90-capacity-generation-source-result",
        "experiment": "v90_capacity_generation_source_inventory",
        "passed": passed,
        "decision": "freeze_source_and_preregister_fresh_model_population" if passed else "stop_V90_before_population_or_model_access",
        "source_integrity": {
            "path": shard["path"],
        ...
        "claim_boundary": "source integrity and text-free structural inventory only; no language score, model access, training, or execution",
    }
