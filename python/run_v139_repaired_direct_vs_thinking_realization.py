...
def main() -> None:
    ...
    if output_dir.exists():
        raise RuntimeError("V139 exact recovery comparison may run only once")
    lock = json.loads(lock_path.read_text())
    if payload_hash({key: value for key, value in lock.items() if key != "lock_payload_sha256"}) != lock["lock_payload_sha256"]:
        raise RuntimeError("V139 lock mismatch")
    dependencies = [key for key in lock if not key.endswith("_sha256") and f"{key}_sha256" in lock]
    for key in dependencies:
        if file_sha256(PROJECT_ROOT / lock[key]) != lock[f"{key}_sha256"]:
            raise RuntimeError(f"V139 dependency drifted: {key}")
    config = lock["config_payload"]
    public_rows = [
        row for row in json.loads((PROJECT_ROOT / lock["public_fixtures"]).read_text())
        if row["split"] == config["population"]["split"]
    ]
    hidden_rows = [
        row for row in json.loads((PROJECT_ROOT / lock["hidden_fixtures"]).read_text())
        if row["split"] == config["population"]["split"]
    ]
    ...
    def evaluate_fixture(row: dict[str, Any]) -> dict[str, Any]:
        ...
        if len(prompt_tokens) > config["prompt"]["maximumPromptTokens"]:
            raise RuntimeError("V139 prompt exceeds frozen budget")
        ...
        parsed = validate_final_answer_v138(
            raw,
            catalog,
            thinking_enabled=condition["enableThinking"],
            prompt_think_opened=condition["promptThinkOpened"],
        )
    ...
    result = run_locked_census_once(
        ...
        result_metadata={
            "schema_version": "139-repaired-direct-vs-thinking-realization-result",
    ...
    print(json.dumps({"completed_condition": result["completed_condition"], "decision": result["decision"], "conditions": summary["conditions"], "access": access}, indent=2, sort_keys=True))
