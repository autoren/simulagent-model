...
from v175_certification_aware_planner_development import (
    ...
    evaluate_safety_gates as evaluate_V175_safety_gates,
)
...
def build_certificate_artifact(
    ...
    for target in targets_artifact["target_cases"]:
        state = state_by_id[target["state_id"]]
...
def evaluate_safety_gates(
    ...
    checks = evaluate_V175_safety_gates(evaluation, access, config)
...
def evaluate_primary_confirmation(
    ...
    exact = _from_payload(
        metrics["exact_certification_adaptive"]["routed_total_risk"]
    )
    return {
        "below_immediate_defer": exact
        < _from_payload(metrics["immediate_defer"]["routed_total_risk"]),
        "below_V167_routed": exact
        < _from_payload(
            metrics["V167_exact_adaptive_horizon2"]["routed_total_risk"]
        ),
        "positive_trusted_completion": _from_payload(
            metrics["exact_certification_adaptive"]["trusted_completion"]
        )
        > 0,
...
def evaluate_strong_confirmation(
    ...
    return {
        ...
        <= _from_payload(
            metrics["greedy_class_information_gain_horizon5"][
                "routed_total_risk"
            ]
        ),
        "no_worse_than_open_loop": exact
        <= _from_payload(
            metrics["optimal_open_loop_subset"]["routed_total_risk"]
        ),
        "no_worse_than_random_order": exact
        <= _from_payload(
            metrics["random_query_order_consensus_stop"]["routed_total_risk"]
        ),
