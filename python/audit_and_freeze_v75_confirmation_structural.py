#!/usr/bin/env python3
"""Run and freeze the single authorized V75 adapter structural audit."""
...
def main() -> None:
    design_lock_path = PROJECT_ROOT / "configs/v75-active-sensing-confirmation-design-lock.json"
    config_path = PROJECT_ROOT / "configs/v75-active-sensing-confirmation-structural.json"
    exporter_path = PROJECT_ROOT / "python/v75_nova_paint_source.py"
    tests_path = PROJECT_ROOT / "python/test_v75_nova_paint_source.py"
    auditor_path = PROJECT_ROOT / "python/audit_and_freeze_v75_confirmation_structural.py"
    attempt_path = PROJECT_ROOT / "outputs/v75-active-sensing-confirmation/structural-attempt.json"
    audit_path = PROJECT_ROOT / "outputs/v75-active-sensing-confirmation/structural-audit.json"
    lock_path = PROJECT_ROOT / "configs/v75-active-sensing-confirmation-structural-lock.json"
    if attempt_path.exists() or lock_path.exists():
        raise RuntimeError("V75 structural audit already attempted")

    attempt_path.parent.mkdir(parents=True, exist_ok=True)
    attempt_path.write_text(
        json.dumps(
            {
                "schema_version": "75-active-sensing-confirmation-structural-attempt",
                "experiment": "v75_nova_paint_structural_audit",
                "attempt_number": 1,
                "maximum_attempts": 1,
    ...
    authorization_ok = bool(
        ...
        and not design_lock["authorization"]["run_policy_outcomes"]
    )
    if not authorization_ok:
        errors.append("V75 design lock does not authorize structural audit")
    ...
    if not tests_ok:
        errors.append(f"V75 structural tests failed: {combined[-1800:]}")
    ...
    checks = {
        ...
        "initial_best_unconditioned_action_matches": bool(
        ...
        "beacon_is_nonharvestable_identity": bool(
            diagnostics["calibration_beacon_harvestable"]
            is gates["requiredCalibrationBeaconHarvestable"]
            and diagnostics["calibration_transition_is_identity"]
        ),
        "source_arrays_are_preserved": diagnostics["source_array_parity"]
        is gates["requiredSourceArrayParity"],
        "point_model_supports_identical": diagnostics["point_model_supports_identical"]
        ...
        "target_inspection_TV_passes": diagnostics["target_inspection_total_variation"]
        >= gates["minimumTargetInspectionTotalVariation"],
        "paired_decision_probability_passes": diagnostics[
        ...
        "forbidden_policy_calls_are_zero": bool(
            ...
            and gates["maximumPriorPolicyOutcomeAccessCount"] == 0
        ),
    }
    ...
    audit = {
        "schema_version": "75-active-sensing-confirmation-structural",
        "experiment": "v75_nova_paint_structural_audit",
        ...
        "decision": (
            ...
            else "freeze_negative_structural_result_and_stop_before_policy_outcomes"
        ),
        "errors": errors,
        "failed_gates": failed,
        "checks": checks,
    ...
    lock = {
        "schema_version": "75-active-sensing-confirmation-structural",
        "experiment": "v75_nova_paint_structural_lock",
        ...
        "authorization": {
            ...
            "implement_and_lock_replication_evaluator": audit["passed"],
