...
def reconstruct(lock: dict[str, Any]) -> dict[str, Any]:
    ...
    objective = lock["config_payload"]["unchangedRobustObjective"]
...
def main() -> None:
    ...
    if output_root.exists():
        raise RuntimeError("V182 formal confirmation may run only once")
    ...
    if (
        ...
        raise RuntimeError("V182 lock mismatch")
    for key in DEPENDENCY_KEYS:
        if file_sha256(PROJECT_ROOT / lock[key]) != lock[f"{key}_sha256"]:
            raise RuntimeError(f"V182 dependency drifted: {key}")
    ...
    certificate_path = output_root / "oracle-certificate-digest.json"
    write_json(
        state_path,
        {
    ...
    write_json(certificate_path, evaluation["certificate_digest"])
    output_integrity = {
        key: {
        ...
        for key, path in {
            ...
            "oracle_certificate_digest": certificate_path,
        }.items()
    }
    result = {
        "schema_version": "182-triple-repetition-robust-planner-fresh-confirmation-result",
