...
from v174_certificate_depth_feasibility_census import evaluate_feasibility, evaluate_gates
...
def reconstruct(lock: dict[str, Any]) -> dict[str, Any]:
    ...
    return evaluate_feasibility(
        ...
        lock["config_payload"]["adaptiveOpportunity"]["horizons"],
    )
...
def main() -> None:
    ...
    if output_root.exists():
        raise RuntimeError("V174 formal census may run only once")
    lock = json.loads(lock_path.read_text())
    if payload_hash({key: value for key, value in lock.items() if key != "lock_payload_sha256"}) != lock["lock_payload_sha256"]:
        raise RuntimeError("V174 lock mismatch")
    for key in DEPENDENCY_KEYS:
        if file_sha256(PROJECT_ROOT / lock[key]) != lock[f"{key}_sha256"]:
            raise RuntimeError(f"V174 dependency drifted: {key}")
    ...
    decision = config["decisionRule"]["ifEveryFeasibilityGatePasses"] if passed else config["decisionRule"]["otherwise"]
    targets_path = output_root / "target-certificate-results.json"
    states_path = output_root / "state-horizon-results.json"
    ...
    output_integrity = {
        ...
        for key, path in {
            ...
            "state_horizon_results": states_path,
            "feasibility_summary": summary_path,
        }.items()
    }
    result = {
        "schema_version": "174-certificate-depth-feasibility-census-result",
