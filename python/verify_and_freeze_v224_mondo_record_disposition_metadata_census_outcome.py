...
from run_v224_mondo_record_disposition_metadata_census import access_ledger, read_jsonl
...
def main() -> None:
    ...
    audit_path = PROJECT_ROOT / "outputs/v224-mondo-record-disposition-metadata-census/outcome-audit.json"
    ...
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V224 outcome is already audited or frozen")
    ...
    checks = {
        ...
        "all_capture_artifact_hashes_are_exact": all(
        ...
        "language_model_protected_and_effect_boundaries_remain_zero": bool(
            metrics["task_language_persistence_count"] == 0
            and all(value == 0 for key, value in access_ledger().items() if key != "formal_census_run_count")
        ),
        "results_document_exists": results_path.is_file(),
    }
    passed = all(checks.values())
    audit = {
        "schema_version": "224-mondo-record-disposition-metadata-census-outcome-audit",
        "experiment": config["experiment"],
        "passed": passed,
        "branch": rebuilt["branch"],
        "decision": "freeze_verified_V224" if passed else "freeze_failed_V224_verification",
        "checks": checks,
        "metrics": metrics,
    ...
    outcome: dict[str, Any] = {
        "schema_version": "224-mondo-record-disposition-metadata-census-outcome-lock",
        "experiment": config["experiment"],
        "outcome": {
            "passed": True,
            "branch": rebuilt["branch"],
