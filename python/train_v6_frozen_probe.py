#!/usr/bin/env python3
"""Train the single preregistered V6 linear probe using only development features."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="outputs/v6-frozen-probe/probe")
    ...
    return [
        {
            ...
            "evidence_pair_id": str(intervention) or None,
            "evidence_variant": str(evidence_variant),
            "mechanic": str(mechanic),
            "gold_ambiguous": bool(gold),
            "score": float(score),
        }
        for record_id, group, pair_id, surface, intervention, evidence_variant, mechanic, gold, score in zip(
            values["ids"],
            values["groups"],
...
def breakdown(rows: list[dict[str, Any]], key: str, threshold: float) -> dict[str, Any]:
    ...
    for value in sorted({str(row[key]) for row in rows}):
        ...
        result[value] = evaluate_binary(
            [row["gold_ambiguous"] for row in selected],
...
def main() -> None:
    ...
    if file_sha256(Path(implementation["path"])) != implementation["sha256"]:
        raise RuntimeError("V6 trainer changed after protocol lock")
    ...
    if metadata["protocol_lock_sha256"] != file_sha256(lock_path):
        raise RuntimeError("V6 features were not extracted under this protocol lock")
    if metadata["mechanic_holdout_records_read"] != 0 or metadata["v3_test_records_read"] != 0:
        raise RuntimeError("V6 development extractor accessed a closed split")
    output_dir = Path(args.output_dir)
    result_path = output_dir / "result.json"
    if result_path.exists():
        raise RuntimeError(f"Probe result already exists; refusing a second training: {result_path}")
    ...
    if train[feature].dtype != np.float32 or calibration[feature].dtype != np.float32:
        raise RuntimeError("V6 features are not float32")
    probe = make_probe(lock["method"]["c_value"], lock["method"]["seed"])
    train_gold = train["gold_ambiguous"].astype(bool)
    # Complete surface triplets share one target. Equal per-row weights therefore give
    # every invariance group equal total weight while directly supervising all surfaces.
    ...
    if train_scores.dtype != np.float32 or calibration_scores.dtype != np.float32:
        raise RuntimeError("V6 probe decision scores are not float32")
    calibration_rows_unthresholded = rows_for(calibration, calibration_scores)
    canonical = [row for row in calibration_rows_unthresholded if row["surface_variant"] == "canonical"]
    ...
    calibration_surface = breakdown(calibration_rows, "surface_variant", threshold)
    calibration_invariance = surface_invariance(calibration_rows, threshold)
    calibration_evidence = evidence_contrasts(calibration_rows, threshold)
    result = {
        "experiment": "v6_fixed_frozen_probe_development",
        ...
        "calibration_by_surface": calibration_surface,
        "calibration_surface_invariance": calibration_invariance,
        "calibration_evidence_contrasts": calibration_evidence,
        "calibration_gate": {
