...
def main() -> None:
    ...
    if source_root.exists() or inventory_root.exists():
        raise RuntimeError("V102 PRESTO source inventory may run only once")
    lock = json.loads(lock_path.read_text())
    if payload_hash(
        {key: value for key, value in lock.items() if key != "lock_payload_sha256"}
    ) != lock["lock_payload_sha256"]:
        raise RuntimeError("V102 PRESTO source lock mismatch")
    dependency_keys = (
    ...
    for key in dependency_keys:
        if file_sha256(PROJECT_ROOT / lock[key]) != lock[f"{key}_sha256"]:
            raise RuntimeError(f"V102 dependency drifted: {key}")

    config = lock["config_payload"]
    archive = config["archive"]
    request = Request(archive["url"], headers={
        "User-Agent": "simulagent-v102-presto-source",
    ...
    if (
        ...
        raise RuntimeError("V102 pinned PRESTO archive identity mismatch")

    source_records, members = parse_presto_archive(
        data, archive["requiredMemberBasenames"]
    )
    inventory = build_presto_context_inventory(source_records, config)
    artifact = {
        "provenance": {
    ...
    checks = evaluate_presto_source_gates(inventory, config)
    ...
    result = {
        "schema_version": "102-presto-context-source-result",
        "experiment": "v102_presto_context_source_inventory",
        "passed": passed,
        "decision": (
            "freeze_PRESTO_source_and_preregister_paired_context_population"
            if passed else "stop_V102_before_population_language_or_model_access"
        ),
        "source_integrity": {
            "url": archive["url"],
            "byte_size": len(data),
            "etag_md5": response_etag,
        ...
        "claim_boundary": (
            "automatic text-free PRESTO context-dependency feasibility only; no selected language, "
