...
def main() -> None:
    ...
    realization_dir = PROJECT_ROOT / "outputs/v158-model-free-hard-tie-routing-policy/model-free-realization"
    result_path = realization_dir / "result.json"
    access_path = realization_dir / "access.json"
    results_doc_path = PROJECT_ROOT / "docs/v158-model-free-hard-tie-routing-policy-results.md"
    verifier_path = PROJECT_ROOT / "python/verify_and_freeze_v158_model_free_hard_tie_routing_policy_outcome.py"
    audit_path = PROJECT_ROOT / "outputs/v158-model-free-hard-tie-routing-policy/outcome-audit.json"
    outcome_path = PROJECT_ROOT / "configs/v158-model-free-hard-tie-routing-policy-outcome-lock.json"
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V158 outcome already frozen")
    if not results_doc_path.is_file():
        raise RuntimeError("write V158 results document before outcome freeze")
    ...
    expected = evaluate(public_requests, metadata, retrieval_catalog, witness_catalog, witness_config, config)
    expected_decision = (
    ...
    expected_result = {
        ...
        "candidate_proposal_field_count": 0, "decision": expected_decision,
    ...
    checks = {
        ...
        "completed_single_development_policy": bool(
            result["completed"] and len(public_requests) == 96 and len(metadata) == 192
            and access["policy_specific_query_score_count"] == 576
        ),
        ...
        "authorization_remains_narrow": bool(
            ...
            and not lock["authorization"]["read_or_score_V157_evaluation"]
            and not lock["authorization"]["change_terms_weights_thresholds_costs_gates_or_comparators"]
            and not lock["authorization"]["run_model_hybrid_API_training_induction_authority_action_or_execution"]
        ),
    }
    passed = all(checks.values())
    audit = {
        "schema_version": "158-model-free-hard-tie-routing-policy-outcome-audit",
        ...
        "comparator_metrics": canonical_json(expected["comparator_metrics"]), "access": access,
    }
    ...
    outcome: dict[str, Any] = {
        "schema_version": "158-model-free-hard-tie-routing-policy-outcome-lock",
        ...
        "authorization": {
            ...
            "design_local_non_authoritative_tie_breaker_protocol_if_policy_qualified": expected["passed"],
            "run_local_tie_breaker_before_separate_preregistration": False,
            "open_or_score_V157_evaluation": False,
            "change_terms_weights_thresholds_costs_gates_or_rerun": False,
