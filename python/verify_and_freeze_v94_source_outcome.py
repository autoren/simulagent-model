...
def main() -> None:
    source_lock_path = PROJECT_ROOT / "configs/v94-global-open-set-source-lock.json"
    result_path = PROJECT_ROOT / "outputs/v94-global-open-set/source-inventory/result.json"
    inventory_path = PROJECT_ROOT / "outputs/v94-global-open-set/source-inventory/global-open-set-inventory.json"
    doc_path = PROJECT_ROOT / "docs/v94-global-open-set-source-results.md"
    verifier_path = PROJECT_ROOT / "python/verify_and_freeze_v94_source_outcome.py"
    audit_path = PROJECT_ROOT / "outputs/v94-global-open-set/source-outcome-audit.json"
    outcome_path = PROJECT_ROOT / "configs/v94-global-open-set-source-outcome-lock.json"
    if audit_path.exists() or outcome_path.exists():
        raise RuntimeError("V94 source outcome is already frozen")
    if not doc_path.is_file():
        raise RuntimeError("write the V94 source result before freezing")
    ...
    reconstructed = build_global_catalog_inventory(
    ...
    checks = {
        ...
        "partition_sets_are_disjoint": bool(
        ...
        "result_and_gate_decision_are_consistent": bool(
            ...
            and result["decision"] == (
                "freeze_source_and_preregister_global_open_set_population"
                if result["passed"] else "stop_V94_before_population_or_model_access"
            )
        ),
    ...
    audit = {
        "schema_version": "94-global-open-set-source-outcome-audit",
        "experiment": "v94_global_open_set_source_outcome_audit",
        "passed": integrity_passed,
        "scientific_source_feasibility_passed": result["passed"],
        "decision": (
            "freeze_positive_V94_global_source_feasibility" if result["passed"]
            else "freeze_negative_V94_global_source_feasibility"
        ) if integrity_passed else "reject_V94_source_outcome",
        "checks": checks,
        "independent_summary": {
            "candidate_count": len(records), "class_counts": dict(sorted(class_counts.items())),
    ...
    if not integrity_passed:
        ...
        raise SystemExit(1)
    dependencies = {
        "source_lock": source_lock_path, "result": result_path, "inventory": inventory_path,
        "source_file": source_path, "verifier": verifier_path, "audit": audit_path,
        "results_document": doc_path,
    }
    outcome: dict[str, Any] = {
        "schema_version": "94-global-open-set-source-outcome-lock",
        "experiment": "v94_global_open_set_source_outcome_lock",
        "outcome": {
            "passed": True,
            "scientific_source_feasibility_passed": result["passed"],
        ...
        "authorization": {
            "modify_or_rerun_V94_source_stage": False,
    ...
    outcome["source_file_git_blob_sha1"] = git_blob_sha1(source_bytes)
