...
def main() -> None:
    ...
    parser.add_argument(
        "--audit", default="outputs/v61-long-horizon-policy-verification/design-audit.json"
    )
    parser.add_argument("--output", default="configs/v61-design-lock.json")
    args = parser.parse_args()
    audit_path, output = tuple(
        (PROJECT_ROOT / value).resolve() for value in (args.audit, args.output)
    )
    if output.exists():
        raise RuntimeError("V61 design is already frozen")
    audit = json.loads(audit_path.read_text())
    config_path = PROJECT_ROOT / audit["config"]
    plan_path = PROJECT_ROOT / audit["preregistration"]
    bound = (
        audit["passed"]
        and audit["config_sha256"] == file_sha256(config_path)
        and audit["preregistration_sha256"] == file_sha256(plan_path)
        and audit["population_seal_sha256"]
    ...
    if not bound:
        raise RuntimeError("V61 design audit is not intact and bound")
    lock = {
        "schema_version": 61,
        "experiment": "v61_design_lock",
        "config": audit["config"],
        ...
        "authorization": {
            "write_and_audit_v61_implementation": True,
            "reconstruct_v60_source_policies": False,
            "build_v61_verification_bundle": False,
            "run_v61_verification": False,
